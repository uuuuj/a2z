...
last_updated: 2026-07-23 (현재 작업 후 중간 취소)
...
처리 오버레이의 **현재 작업 후 취소** 버튼을 누르면 실행 중인 SDK 단일 호출은 끝까지 완료하고 BOM·간섭검사·Osnap·치수·시트 생성 사이 체크포인트에서 중단한다. 취소 뒤에는 부분 시트·2D·3D·치수·Osnap 상태를 정리하고 두 자동 작업 버튼을 다시 활성화한다.
...
| 3 | **취소 가능한 오버레이 표시** | UI | 취소 상태 초기화와 재진입 차단 후 `ShowBusyOverlay("BOM 수집 중...")`. 오버레이에 **현재 작업 후 취소** 버튼 표시 |
...
| # | 단계 | 주체 | 설명 |
|---|---|---|---|
| 12 | Osnap 수집 | Form1 | 수집 전·후 취소 확인 → `ShowBusyOverlay("Osnap 수집 중...")` → `CollectAllOsnap()` → `_autoProcessOsnapSuccess` 저장 (`osnapPointsWithNames`·`lvOsnap` + **T-032**: 부재별 맵 `_lastCollectedNodeOsnapMap`도 같이 채움) |
