...
last_updated: 2026-07-24 (대용량 경고·BOM 세분화 취소)
...
처리 오버레이의 **취소** 버튼을 누르면 실행 중인 SDK 단일 호출은 끝까지 완료하고 가장 가까운 안전 체크포인트에서 중단한다. BOM·홀 정보·Osnap 수집은 부재마다, 전체 BODY 가시성 판정과 목록 구성은 200개 단위로 진행 수를 갱신하고 UI 입력을 처리한다. 취소 뒤에는 부분 BOM·시트·2D·3D·치수·Osnap 상태를 정리하고 관련 출력 버튼을 원래 상태로 복원한다.
...
flowchart TD
    ...
    B2 --> B3[현재 대상 BODY 확인<br/>진행 수 표시]
    B3 --> WARN{대상 5,000개 이상?}
    WARN -- 예 --> ASK{계속 실행?}
    ASK -- 아니오 --> STOP[상태 복원 후 종료]
    ASK -- 예 --> C[CollectBOMData<br/>부재마다 취소 확인]
    WARN -- 아니오 --> C
    C --> D{bomList > 0?}
    D -- 아니오 --> E01[알림 후 종료]
...
| 3 | **취소 가능한 오버레이 표시** | UI | 취소 상태 초기화와 재진입 차단 후 `ShowBusyOverlay("BOM 대상 확인 중...")`. 오버레이에 **취소** 버튼 표시 |
| 3.5 | 대상 확인·대용량 경고 | Form1 | 전체 BODY를 200개 단위로 스캔해 현재 선택/visible 대상을 확정한다. 5,000개 이상이면 STRU 격리를 권장하고 `계속/취소`를 묻는다. 기본 선택은 취소 |
| 4 | BOM 재수집 | Form1 | 확정한 대상 목록을 `CollectBOMData(targetNodes)`에 전달한다. 부재·홀 정보는 매 부재, ListView는 200행 단위로 진행 문구 갱신·취소 확인 |
| 4.5 | 취소 확인 | Form1 | BOM 수집 후와 간섭검사 시작 전에 요청을 확인. 요청 시 부분 상태 정리 후 종료 |
...
| # | 단계 | 주체 | 설명 |
|---|---|---|---|
| 9 | 취소 확인·Clash 결과 수집 | Form1 | 각 무창 검사 완료 이벤트에서 취소 요청을 먼저 확인. 요청이면 다음 검사 ID를 시작하지 않고 큐를 정리. 계속할 때만 `ClashTest` 순회 → `GetResultItem(PART)` |
...
| # | 단계 | 주체 | 설명 |
|---|---|---|---|
| 12 | Osnap 수집 | Form1 | 수집 전·후와 매 부재마다 취소 확인 → 현재/전체 진행 수 표시 → `CollectAllOsnap()` → `_autoProcessOsnapSuccess` 저장 (`osnapPointsWithNames`·`lvOsnap` + **T-032**: 부재별 맵 `_lastCollectedNodeOsnapMap`도 같이 채움) |
| 13 | **치수 계산 통합** (T-028 + T-032 최적화) | Form1 | 계산 전·후 취소 확인 → `ShowBusyOverlay("치수 계산 중...")` → visible 부재 목록 산출 → `ComputeViewDimensionsForMembers(visibleMembers, null, 0.5f, _lastCollectedNodeOsnapMap)` 호출. **`_lastCollectedNodeOsnapMap` 전달로 GetOsnapPoint 중복 호출 제거** (T-032). 3뷰 × 2축 = 6조합 치수 생성 + 중복 제거 |
...
| 18 | 시트 생성·조회 준비 | Form1 | 시트 생성 전·후 취소 확인 → `GenerateDrawingSheets()` — 시트 구성을 만든 뒤 일반·설치 시트 치수와 모든 시트 BOM을 사전 준비 |
| 19 | 작업 종료 | UI | 오버레이 해제, 취소 상태 초기화, 메인 치수 추출·도면 일괄 출력 버튼 재활성화 |

> 최종 결과 요약 알림은 [Clash 완료 콜백](../간섭검사/간섭검사%20완료%20이벤트.md)에서 표시됨
...
| 취소 시점 | 처리 |
|---|---|
| BOM·Osnap·치수·시트 작업 전 | 다음 SDK 호출을 시작하지 않고 즉시 정리 |
| SDK 단일 호출 실행 중 | 호출 반환 뒤 다음 체크포인트에서 정리 |
| 비동기 간섭검사 실행 중 | 현재 검사 완료 이벤트까지 기다린 뒤 다음 검사 ID를 시작하지 않음 |

취소 시 부분 시트 목록, 치수·Osnap 캐시, 2D Canvas/Object, 3D Note/Measure/ShapeDrawing을 정리한다. 강제 스레드 중단은 사용하지 않는다.

### [분기 F] 대용량 대상 사전 확인

| 조건 | 처리 |
|---|---|
| 대상 BODY 5,000개 미만 | 별도 확인 없이 BOM 수집 계속 |
| 대상 BODY 5,000개 이상 + 계속 | 장시간 소요 경고를 확인한 뒤 BOM 수집 계속 |
| 대상 BODY 5,000개 이상 + 취소 | BOM을 만들기 전에 오버레이·버튼 상태를 복원하고 종료 |
...
| E05 | 사용자 취소 요청 | 체크포인트에서 정리 후 return | MessageBox "치수 추출을 취소했습니다" + 중단 위치 | 부분 2D·3D·시트·치수 상태 제거, 다음 실행 가능 |
| E06 | 대상 BODY 5,000개 이상 | 사용자 선택에 따라 계속 또는 종료 | STRU 격리 권장과 예상 장시간 안내 | 취소 선택 시 기존 BOM·도면 상태를 새로 만들지 않음 |
...
| `_cancelRequested`, `_mainDimensionInProgress` | false | 처리 중 true, 정상·취소·예외 종료 시 false |
...
## 8. 후행 기능 (Chained)
- [Clash 완료 콜백](../간섭검사/간섭검사%20완료%20이벤트.md) — 자동 호출
- 이후 [시트 자동 분할](../도면시트/시트%20자동%20생성.md) — Clash 결과 있으면 자동
- [축별 치수 필터](../치수/X축%20치수%20표시.md)

## 9. 관련 링크
- 코드 구현: [Form1.BOM.cs:L332](../../code-reference/form1-bom.md#btnMainDimension_Click)
...
| 날짜 | 변경 내용 | 작성자 |
|---|---|---|
| 2026-07-24 | 관련: T-091, GitHub issue #51 — 취소 버튼 문구를 단순화하고 BODY 가시성 스캔·BOM·홀·Osnap·목록 루프에 진행 수와 UI 체크포인트를 추가. BOM·홀·Osnap은 매 부재마다 취소를 확인한다. 대상 BODY 5,000개 이상은 STRU 격리를 권장하는 사전 확인을 표시하며 취소 시 BOM·부분 도면 상태와 출력 컨트롤을 정리·복원 | Codex |
| 2026-07-23 | 처리 오버레이에 **현재 작업 후 취소** 버튼을 추가. BOM·비동기 간섭검사·Osnap·치수 계산·시트 생성 전후 체크포인트에서 협력적으로 중단하고 부분 시트·2D·3D·치수 상태와 버튼을 복원 | Codex |
...
| 2026-05-06 | **T-058**: `ShowAllDimensions`에서 `AlignDistanceTextPosition = 0 → 2` (보조선 바깥 배치). 회사 doc "개발 요청 — 상 5" 사양 반영, 좁은 치수에서 텍스트가 보조선 사이를 침범하는 문제 회피. SDK 글로벌 옵션이라 모든 치수 일괄 적용. 통합 사양: [docs/기술 노트/치수 텍스트 위치.md](../../기술%20노트/치수%20텍스트%20위치.md) | Claude |
