...
## 1. 개요
**원클릭 통합 처리 버튼**: BOM 수집 → **Clash 검사 → 연결성 판정** → Osnap 수집 → X/Y/Z 체인 치수 계산 → `lvDimension` 목록 갱신 → 요약 → 시트 생성. Clash가 먼저 수행되고 **모든 부재가 한 덩어리로 연결되어 있어야** Osnap·치수 파이프라인이 이어진다. 제작도 ISO에 연결 부재를 점선으로 넣을 수 있도록 모델 전체 Bounding Box에서 근접 후보를 먼저 추린 뒤 `현재 BOM 대상 vs 근접 후보` 그룹 검사도 함께 등록한다.
