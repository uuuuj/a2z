...
last_updated: 2026-07-29 (ViewMode 보존)
...
- 대비 참고: [모델 파일 열기](./모델%20열기.md) (초기화 블록이 거의 동일하되 파일 다이얼로그 포함)
...
| 날짜 | 변경 내용 | 작성자 |
|---|---|---|
| 2026-07-29 | 관련: #49 · #55 — 초기화가 화면 구성을 바꾸지 않도록, 진입 시점 `ViewMode`를 잡아뒀다가 `Clear2DView()` 뒤에 `RestoreViewMode()`로 되돌린다. `Model.Open`과 `Clear2DView`가 각각 2D 뷰를 강제로 여는 것이 원인이었다 | Claude |
