...
선택 STRU 전체 범위 치수는 문맥용으로 유지한다. 위치 치수는 Target Body 길이축과 가장 가까운 전역 X/Y/Z축을 사용하며 그 축이 화면에 보이는 두 직교 뷰에만 생성한다. 0.5mm 이하이거나 깊이 방향으로만 차이가 나는 뷰에는 표시하지 않는다.
...
| `PreparedDimensions` | 선택 STRU 전체 범위와 Target Body 끝단→Connected Body 모서리 필수 치수 저장 |
