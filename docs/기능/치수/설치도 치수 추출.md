...
설치도는 선택한 STRU 전체를 실선으로, 그 STRU에 직접 연결된 외부 Part만 점선으로 표시한다. Assembly 정보는 ISO 접합 노트의 이름 문맥으로만 사용한다. 치수는 단순 BBox 경계가 아니라 다음 두 기준을 결합한다.

- 선택 STRU의 `LINE` 시작·끝점과 `POINT` 중심 Osnap에서 주축·보조축 MIN/MAX를 선택한다.
...
|---|---|---|---|---|
| X 보기 | YZ | Z | Y | 선택 STRU의 Z/Y 전체 범위 + 연결 Part 끝단↔접합 영역 체인 |
| Y 보기 | XZ | Z | X | 선택 STRU의 Z/X 전체 범위 + 연결 Part 끝단↔접합 영역 체인 |
| Z 보기(평면도) | XY | Y | X | 선택 STRU의 Y/X 전체 범위 + 연결 Part 끝단↔접합 영역 체인 |
...
| 외부 연결 Part 없음 | 선택 STRU만 표시하고 설치 치수는 STRU 전체 범위만 생성 |
...
| `PreparedDimensions` | 선택 STRU 전체 범위와 연결 Part 접합 위치 필수 치수 저장. 연결 Assembly 전체 범위는 생성하지 않음 |
