...
**뷰 배정** (`viewAxes`): 각 성분을 그 축이 **화면 평면에 있는**(=화면에 실제 길이로 보이는) 직교 뷰에만 배정한다. X 성분은 정면도(시선 +Y)·평면도(시선 −Z), Y 성분은 측면도(시선 −X)·평면도, Z 성분은 측면도·정면도에 표시한다. 그 축을 정면으로 마주보는 뷰(X 성분 ↔ 시선 −X)는 축이 깊이 방향이라 치수의 두 끝점이 화면상 겹쳐 **보조선이 하나로 합쳐지고 치수가 뭉개진다**(2026-07-23 실기 PDF로 확인). 그래서 그 뷰에는 배정하지 않는다.

> ⚠ **측면도(시선 −X)에 X축 형강 길이 치수는 넣을 수 없다**: 정면도(시선 +Y)에서는 X 길이가 좌우 화면축에 보이지만, 측면도에서는 X가 깊이 방향으로 들어간다. "형강 끝→Tray" 거리는 길이 방향 치수라 정면도·평면도에만 나오고 측면도에는 물리적으로 못 그린다. (세 뷰 모두 배정을 시도했다 측면도에서 겹친 보조선으로 잘못 그려져 폐기 — 2026-07-23.)
...
- 2D/PDF는 `keepCamera: true`로 **표준 카메라 뒤 적용한 ORIENTATION/참조축 회전을 유지한 채** 측정·보조선을 만든다. `ShowAllDimensions`가 다시 카메라를 옮기면 화면축 회전이 초기화되어 모델 캡처와 치수의 2D 변환이 어긋날 수 있다.
...
- [Osnap 선별 기준](../../기술%20노트/Osnap%20기준.md)
...
| 날짜 | 변경 내용 | 작성자 |
|---|---|---|
| 2026-08-21 | 직교 뷰를 평면도(시선 −Z)·측면도(시선 −X)·정면도(시선 +Y)로 정정하고 `keepCamera` 설명을 표준 카메라 이후 화면축 회전 보존 기준으로 갱신 | Codex |
| 2026-07-23 | (세 뷰 배정 시도 → 폐기) 위치 치수를 세 뷰 모두 배정했더니 측면도에서 형강 길이축(X)이 깊이 방향이라 보조선이 겹쳐 치수가 뭉개짐(실기 PDF). 2뷰 배정으로 복원 — 길이 치수는 X축이 화면에 보이는 정면도·평면도에만, 측면도(끝면 방향)엔 물리적으로 불가 | Claude |
