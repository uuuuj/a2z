...
| 뷰 | 화면 평면 | 표시 성분 |
|---|---|---|
| X 보기 | YZ | 끝단↔모서리의 Z·Y 성분 치수 |
| Y 보기 | XZ | 끝단↔모서리의 Z·X 성분 치수 |
| Z 보기(평면도) | XY | 끝단↔모서리의 Y·X 성분 치수 |

선택 STRU 전체 범위 치수는 생성하지 않는다. 끝단↔모서리 벡터의 **X/Y/Z 세 축 성분 치수를 모두** 만들고, 각 직교 뷰는 자기 화면 평면에 보이는 두 성분만 표시한다 (2026-07-23 — 이전에는 Target Body 길이축 성분만 만들어 길이축과 직교하는 뷰, 예: 수직 부재의 평면도가 빈 뷰가 됐음). 성분이 0.5mm 이하인 축은 생성하지 않으며, 각 앵커의 세 성분은 `[설치치수]` 로그로 남긴다.
...
| 날짜 | 변경 내용 | 작성자 |
|---|---|---|
| 2026-07-23 | 직교 뷰 치수를 주축 1성분에서 끝단↔모서리 X/Y/Z 3축 성분으로 확장 (수직 부재의 평면도가 빈 뷰가 되던 문제 해소). 2D 변환 시 캡처 카메라 유지(`keepCamera`)로 치수가 모델 반대편에 찍히던 좌우 거울 반전 수정. 치수선 baseline을 STRU BBox·치수 끝점 합집합으로 확장해 보조선이 치수선을 지나치던 오버슛 제거 | Claude |
