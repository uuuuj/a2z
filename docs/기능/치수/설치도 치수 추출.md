...
설치도 위치 치수의 목적은 **연결부재가 기준부재의 어느 위치에 붙는지** 보여주는 것이다. 따라서 치수는 "기준부재가 유의미하게 길게 뻗은 축"에만 만든다 — 방향별 특례 없이 **축 게이트 + 뷰 게이트** 두 단계로만 결정한다 (issue #12, 사용자 확정 2026-07-23).

**축 게이트** (`BuildInstallationPlacementAnchor`): Target Body Osnap(또는 BBox fallback)의 축별 extent(max−min)를 재고, `extent ≥ 최대 extent × InstallationAxisExtentRatio(0.25)` **그리고** `extent ≥ InstallationAxisMinExtent(30mm)`를 만족하는 축만 채택한다. 단 주축(최장축)은 무조건 채택해 검증된 길이 치수의 회귀를 막는다. 이렇게 하면 **판 두께·법선 축**(연결부재가 어셈블리 간격 ~1mm 떨어져 뻗는 방향)과 **가늘고 긴 부재의 단면축**(예: 50×50×320 막대의 50mm X·Y — 연결부재가 옆으로 떨어진 거리가 성분으로 잡히던 것)이 걸러진다. 결과적으로 사용자 원칙("연결부재와 나란한 치수 불필요, 기준부재 길이 방향만")대로 겹쳐 보이던 1mm 틈 치수·불필요 수평 성분이 사라진다. 채택된 축마다 그 축 기준으로 끝단을 다시 고른다(`SelectInstallationTargetEndForAxis`) — 주축 끝단(예: 세로 30mm)을 폭 방향 치수에 그대로 쓰면 안 되기 때문. 임계값은 실기 튜닝 전제이며 `[설치치수축]` 로그(축별 extent·채택 목록)로 근거를 남긴다.

**성분 상한** (`BuildInstallationPlacementAnchor`, 2026-07-23 실기): 설치도는 "연결부재가 부재 위 **어느 위치**에 붙는지"를 보이므로 위치는 부재 span 안이어야 한다. 연결부재가 큰 부재라 그 축으로 멀리 뻗으면 접합 osnap이 부재 밖에 잡히고(연결부재 전체 Osnap에서 접합 최근접점을 고르므로), 그 성분은 "부재 위 위치"가 아니라 **먼 부재까지의 거리**가 되어 작은 부재 프레임을 벗어난다. 그래서 성분값이 그 축 부재 extent × `InstallationComponentExtentCapFactor(1.25)`를 넘으면 제외한다. 예: 50mm 두께 축에 잡힌 성분 147·950은 부재(50)를 크게 넘으므로 도면 밖 치수 → 제외. `[설치치수축]` 로그의 `상한제외`에 축·성분·기준을 남긴다. (축 게이트가 축을 채택해도 이 상한이 실제 값을 최종 판정)

**성분 축정렬 투영** (`ComputeInstallationDimensions`): 각 성분 치수는 기준 끝단과 연결 모서리 두 점이 여러 축에서 동시에 벌어져 있으므로(예: X로 147·Z로 30), 끝점을 그 성분 축으로만 벌어지게 투영한다 — 기준 끝단의 나머지 두 좌표를 공유하고 성분 축만 연결 모서리 좌표로 바꾼다. 투영을 안 하면 한 축 치수를 그릴 때 보조선이 부재를 가로질러 연결부재 쪽까지 뻗어 큰 공백이 생긴다(2026-07-23 실기 피드백).

**뷰 게이트** (`viewAxes`): 채택된 축 성분을 그 축이 화면에 보이는 직교 뷰에만 표시한다.
...
예: 세워진 판형(길이축 + 폭축 채택)은 측면 뷰에 길이 치수, 평면도에 폭 방향 위치 치수가 나온다. 가늘고 긴 기둥(길이축만 채택)은 평면도에 잴 긴 방향이 없어 비는 게 정상이다. 선택 STRU 전체 범위 치수는 생성하지 않고, 축 게이트를 통과해도 성분이 `InstallationMinComponent(3mm)` 이하면 미소 성분으로 걸러 겹친 선을 이중 차단한다.
...
| 날짜 | 변경 내용 | 작성자 |
|---|---|---|
| 2026-07-23 | **성분 상한 도입**(issue #12 실기 3차): 성분값(끝단→연결 접합점 거리)이 부재 extent×1.25를 넘으면 제외. 연결부재가 큰 부재라 그 축으로 멀리 뻗어 접합 osnap이 부재 밖에 잡히면 성분이 위치가 아니라 먼 거리가 되어 도면을 벗어나던 것(50mm 축에 성분 950 등) 차단. 축 게이트는 "부재가 긴 축"만, 상한은 "연결부재가 부재 span 안"을 판정 | Claude |
