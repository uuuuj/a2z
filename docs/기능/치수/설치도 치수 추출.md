...
**위치 기준 = 접합점** (`BuildInstallationPlacementAnchor`): 위치를 연결부재 osnap이 아니라 접합선 대표점(centroid)까지 잰다. 연결부재가 큰 부재라 그 osnap이 접합부에서 멀리 떨어져 있어도(옛 실측 147·950), 접합점 자체는 기준부재 위라 위치가 부재를 벗어날 수 없다. (옛 방식은 연결부재 전체 Osnap에서 최근접점을 골라 큰 연결부재면 부재 밖 950까지 잡혀 도면을 벗어났고, 이를 1.25배 상한으로 막았음 — 상한 폐기.)
