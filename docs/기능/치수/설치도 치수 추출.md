...
**축 게이트** (`BuildInstallationPlacementAnchor`): Target Body Osnap(또는 BBox fallback)의 축별 extent(max−min)를 재고, `extent ≥ 최대 extent × InstallationAxisExtentRatio(0.15)` **그리고** `extent ≥ InstallationAxisMinExtent(30mm)`를 만족하는 축만 채택한다. 단 주축(최장축)은 무조건 채택해 검증된 길이 치수의 회귀를 막는다. 이렇게 하면 **판 두께·법선 축**(연결부재가 어셈블리 간격 ~1mm 떨어져 뻗는 방향)이 걸러져, 도면에서 보조선 두 개가 겹쳐 보이던 **의미 없는 1mm 틈 치수**와 설치 위치와 무관한 수평 성분이 함께 사라진다. 채택된 축마다 그 축 기준으로 끝단을 다시 고른다(`SelectInstallationTargetEndForAxis`) — 주축 끝단(예: 세로 30mm)을 폭 방향 치수에 그대로 쓰면 안 되기 때문. 임계값은 실기 튜닝 전제이며 `[설치치수축]` 로그(축별 extent·채택 목록)로 근거를 남긴다.
...
| 날짜 | 변경 내용 | 작성자 |
|---|---|---|
| 2026-07-23 | **축 게이트 도입** (issue #12): 위치 치수를 부재가 유의미하게 긴 축(extent ≥ 최대×0.15 且 ≥30mm, 주축 무조건)에만 생성. 채택 축마다 끝단 재선정(`SelectInstallationTargetEndForAxis`). 판 두께·법선 축을 배제해 겹쳐 보이던 1mm 어셈블리 틈 치수·불필요 수평 성분 제거, 판형 부재 평면도 폭 방향 위치 치수는 유지. 미소 성분 가드 0.5→3mm. 직전 3축 무조건 생성을 대체 | Claude |
