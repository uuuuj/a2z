...
| 12 | 직렬 후속 실행 | Form1/SDK | 각 완료 이벤트에서 다음 ID를 같은 방식으로 시작하고, 마지막 ID 완료 때만 전체 결과 처리 |
...
| 조건 | 처리 |
|---|---|
| 첫 ID 시작 `true` | 비동기 시작. 완료 이벤트마다 다음 ID를 직렬 실행 |
| 첫 ID 시작 `false` | 즉시 실패 반환, E03 |
| 후속 ID 시작 `false` | 큐 초기화·오버레이 해제. 수동/일반 경로는 재실행 안내, STRU 일괄 경로는 기존 시작 실패와 같은 최소 시트 생성 fallback |
...
| E04 | 후속 ID 시작 실패 | 완료 콜백에서 큐 중단 | 재실행 안내. STRU 일괄 경로는 최소 시트 생성 fallback | 일부 검사 결과만 존재 |
