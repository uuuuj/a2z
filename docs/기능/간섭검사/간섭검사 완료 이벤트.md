...
flowchart TD
    ...
    A1 -- 예 --> A2[다음 ID progressForm=false 실행 후 반환]
...
| # | 단계 | 주체 | 설명 |
|---|---|---|---|
| 1 | 무창 큐 진행 | Form1 | 완료 ID가 현재 실행 ID인지 확인. 대기 ID가 있으면 다음 `PerformInterferenceCheck(id, false)`를 시작하고 반환 |
...
| 대기 ID가 남음 | 다음 ID를 진행창 없이 실행하고 결과 집계는 보류 |
...
| E04 | 후속 ID의 무창 실행 시작 실패 | 큐 리셋·오버레이 해제 | 수동/일반 경로는 재실행 안내. STRU 일괄 경로는 최소 시트 생성 fallback | 일부 테스트 결과만 존재하며 전체 결과 집계는 수행하지 않음 |
