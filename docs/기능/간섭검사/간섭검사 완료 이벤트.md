...
## 1. 개요
VIZCore3D가 모든 ClashTest 실행을 완료했을 때 호출된다. 대상 내부 결과와 제작도 연결 부재 결과를 테스트 이름으로 구분해 각각 중복 제거·정렬한다. 대상 내부 결과만 `clashList`에 저장해 연결성 판정에 사용하고, 제작도 연결 결과는 전용 리스트·Part 집합에 저장해 `lvClash`의 `[연결]` 행과 제작도 ISO 점선 대상에 사용한다. 연결성 판정을 통과할 때만 `CompleteMainDimensionPostClash`(Osnap → 치수 → 요약 → 시트)를 호출한다.
...
| 제작도 연결 결과 | 비어있음 | 근접 후보 ClashData와 상대 Part 인덱스 집합 | 동일 |
