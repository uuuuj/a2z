...
| `AO38:AW41` | `{View_6}` | `ISO_North_Arrow.png` |

`PlaceImageInTemplateArea`가 이미지 객체 생성 전후의 2D 객체 목록을 비교해 새 객체를 찾고, 영역 폭·높이에 맞춰 종횡비를 유지한 뒤 영역 중앙으로 이동한다. 파일·영역 누락이나 SDK 자동 생성 실패는 로그를 남기고 모델 도면 출력은 계속한다.
