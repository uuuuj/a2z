...
- **ISO 점선 두 겹 표현 (2026-07-21 — 소프트힐스 신 API 3종)**: 조립도(Sheet2+, `BaseMemberIndex ≥ 0`)는 전체 구조를 LONG_DASHED 점선 배경으로 깔고 **기준부재만 실선**(프레임 = 전체 fit), 제작도(Sheet1, `-1`)는 시트 부재 실선 + **간섭검사로 붙은 시트 밖 부재를 점선**. 순서 = 점선 먼저 캡처 → (제작도) **점선을 시트 부재 영역+여백만 남기고 Crop** → LONG_DASHED 지정 → 점선 객체를 영역에 fit·이동 → 실선 캡처 → 점선 스케일로 통일 → `Match2DObjectsTo3DObjectPosition(실선, 점선)` 정합. 두 도면 모두 점선이 정합 기준이며, 제작도 점선은 CropFit 결과가 시트 프레임 역할을 한다. 점선 대상이 없으면 단일 캡처 폴백.
  ...
  - **이웃 산출**: 자동 치수·도면 출력의 간섭검사에서 현재 제작 대상 대 모델 나머지 Part 그룹 검사 1건을 추가한다. `GetClashNeighborPartsOutsideSheet`는 `clashList` Part 쌍의 한쪽만 시트 소속이면 반대쪽을 수집한다. 기존처럼 가시 대상 내부만 검사해 이웃이 항상 0개가 되던 원인을 제거했다. 풍선은 현행 유지.
