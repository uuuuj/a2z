...
- **Input 슬롯 선초기화 (2026-07-10)**: `data` dict를 1..199 빈 문자열로 선초기화 — 코드가 안 채우는 슬롯의 `{Input_N}` 태그가 도면에 그대로 노출되는 것 방지 (가공도와 동일 패턴). 비-BOM 슬롯: 164=Note 본문, 165~168=PAINT CODE/DP No 구역, 169=TAG NO, 170~199=Rev 이력 표(5행×6열). 코드는 1~3(도면정보)과 BOM만 실값을 채운다. 2026-07-21부터 비-BOM 슬롯 초기값은 빈칸이 아니라 공백(`" "`) — 아래 괘선 제거 항목의 보호 장치.
...
- **빈 칸 괘선 제거 (2026-07-21)**: `ImportExcelWithData` 직후 `RemoveEmptyTemplateBorders(0.1f, RowAndColumn)`(SDK 1.0.26.716) 호출 — 내용 없는 공백 셀(미기재 BOM 행 등)의 테두리 선을 SDK가 지운다. 전역 동작(영역 지정 오버로드 없음 — DLL 전수 확인)이라 1차 실기에서 Rev 표 등 빈 Input 칸 괘선까지 전부 소실 → **비-BOM 슬롯을 공백(`" "`)으로 채워 '내용 있음' 위장**, BOM 빈 행만 제거 대상으로 선별 (위 선초기화 참조). 태그 없는 빈 칸(CLIENT 예약 박스 등)은 코드로 위장 불가 — 필요 시 템플릿 셀에 공백 문자 직접 입력. 공백이 '내용'으로 인정되는지는 2차 실기 확인 대상.
