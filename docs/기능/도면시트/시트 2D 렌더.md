...
last_updated: 2026-08-21 (평면·측면·정면도 카메라 정합)
...
| 2 | Z 평면도 (시선 −Z) | `ShowAllDimensions("Z", true, estScale)` → **`MarkNonRightAngles("Z")`** → 설치도 Target Body 끝단→Connected Body 모서리 위치 치수 → `shapeDrawingIds` → `Add2DObjectFromShapeDrawing` + `Add2DMeasureFrom3DMeasure` | × 0.65 |
| 3 | X 측면도 (시선 −X) | 동일 — `ShowAllDimensions("X", …)` + `MarkNonRightAngles("X")`; A/A1 접합점 기호 없음 | × 0.70 |
| 4 | Y 정면도 (시선 +Y) | 동일 — `ShowAllDimensions("Y", …)` + `MarkNonRightAngles("Y")`; A/A1 접합점 기호 없음 | × 0.70 |
...
- **2단 텍스트 슬라이드 (2026-07-21 사용자 사양, 2026-08-21 카메라 정정)**: 2단에 그려지는 치수(승격 작은 치수 + 기존 2단 체인)는 텍스트를 치수선 방향으로 **종이 절대 2.5mm** 일괄 이동 — 가로 치수=화면 오른쪽, 세로 치수=화면 위(SDK가 치수선 방향 성분만 반영해 세로는 오른쪽 불가). 표준 뷰 매핑은 X뷰 right=+Y·up=+Z / Y뷰 right=+X·up=+Z / Z뷰 right=+X·up=+Y이며, 세 뷰 모두 보이는 치수축의 양(+) 방향을 쓴다. 상수 `Lvl2TextSlideCanvas=2.5`. 진단 로그 `[TextSlide]`.
...
- **신 템플릿 전환 + BOM 20행 (2026-07-10)**: 사용자 제작 `제작도_도면_1.xlsx`(297×210 그리드, 1칸=1mm, 제도 교본 여백)로 **코드 참조 전환 완료**. View_1~4(ISO/Z/X/Y) + View_5(North Arrow)/View_6(CLIENT 로고 예약)/View_7(ISO North Arrow) + `{Image}`(CONTRACTOR 로고) + Input_1~199. **BOM은 8열×20행, 슬롯은 열별 20연속**: No=4~23, ITEM=24~43, MATERIAL=44~63, SIZE=64~83, Q'TY=84~103, T/W=104~123, MA=124~143, FA=144~163. 부재가 20개를 넘으면 20행까지만 표시 + `P2 BOM n행 중 20행만 표시` 로그. 구 템플릿(`사용자템플릿_엑셀_제작도.xlsx`, BOM 15행 체계)은 신 번호와 호환되지 않았고, 짝이던 "엑셀 PoC" 버튼과 함께 정리됨 — 템플릿 파일은 커밋 `0e94aa9`에서, 버튼·핸들러는 2026-07-28에 제거 (#71).
- **ISO 점선 두 겹 표현 (2026-08-04 #202 개정)**: 조립도(Sheet2+, `BaseMemberIndex ≥ 0`)·제작도(Sheet1, `-1`)·설치도(`-2`) 세 종류가 **같은 순서**를 쓴다 — 시트 부재+주변을 함께 점선 배경으로 캡처 → 시트 부재 노드 기준 CropFit → LONG_DASHED → 점선 fit·이동 → 시트 부재만 실선 캡처 → `Match(실선, 점선)`. 점선 대상이 없으면 단일 실선 캡처로 폴백한다.
  - 실선 = 그 시트에 표현되는 부재(`sheet.MemberIndices`), 점선 = 그 외. 조립도의 점선은 `전역 bomList − 시트 부재`, 제작도는 간섭검사로 붙은 시트 밖 부재, 설치도는 직접 연결 외부 Part다.
  - **배율 기준은 항상 Crop된 점선 배경**이다. Crop이 "시트 부재 ± 여백"으로 범위를 묶으므로 배율이 시트마다 흔들리지 않는다.
  - **주변 Crop**: `CropFit2DViewObjectByNodeIDs(시트부재+주변 점선객체, 시트부재 3D노드, cropOffset)` — SDK 예제처럼 Crop 기준 노드를 대상 2D 객체 안에 포함한 상태에서 호출한다. 긴 주변 부재는 붙은 부위 주변만 남고, 점선으로 함께 들어간 시트 부재는 다음 실선 캡처가 위에서 덮는다. `cropOffset`(여백 비율) = 상수 `IsoNeighborCropOffset`(현 0.5f, 실기 튜닝값 — 0이면 시트 영역에 딱 맞고 클수록 맥락 더 보임). 제작도·설치도·조립도가 같은 값을 쓴다.
...
| 날짜 | 변경 내용 | 작성자 |
|---|---|---|
| 2026-08-21 | 평면도/측면도/정면도 카메라를 각각 `Z_PLUS`/`X_PLUS`/`Y_MINUS`로 통일. Y뷰 화면 right가 +X가 되므로 모델 수평 이동과 2단 치수 텍스트의 Y 전용 부호 반전을 제거하고, 각도 진단 투영축도 동일 기준으로 명시. 도면 라벨 부호 표기는 담당자 확인 전까지 기존 템플릿 유지 | Codex |
| 2026-08-04 | **조립도 ISO 배율·배치를 시트 부재 기준 CropFit으로 통일** (관련: #202). 조립도 ISO의 `isoFitByDashed`를 `false`로 바꿔 설치도·제작도와 같은 경로(배경에 시트 부재 포함 캡처 → 시트 부재 기준 CropFit → 그 결과로 fit)를 타게 했다. 종전의 "전체(점선+실선) 기준 fit"은 배율을 점선 배경 bbox로 정해, 점선 구성이 시트마다 달라지면 배율이 요동쳤다 (2026-08-04 실기 5장: 0.127/0.286/0.286/0.135/0.182 — 배율이 튄 장은 실선이 템플릿을 뚫고, 줄어든 장은 BOM 부재 강조가 사라짐). 카메라도 시트 부재 기준으로 바뀌어 풍선 누락도 함께 해소 예상. 사내 실기 대기 | Claude |
