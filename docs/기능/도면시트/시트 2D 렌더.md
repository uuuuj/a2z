...
| 13 | 풍선 배치 | Form1 | 번호 풍선 생성, `balloonOverrides`로 충돌 회피 |
...
- **설치도 4개 뷰 두 겹 표현 (2026-07-22)**: 설치도(`BaseMemberIndex == -2`)는 ISO뿐 아니라 Z/X/Y에서도 선택 STRU 전체를 실선, 직접 연결된 외부 Part만 LONG_DASHED 점선으로 캡처한다. 점선 캡처에는 Crop 기준인 선택 STRU도 함께 넣고 `CropFit2DViewObjectByNodeIDs(..., sheet.MemberIndices, IsoNeighborCropOffset)`로 STRU 영역+여백만 남긴다. 이어서 점선 객체를 STRU 기준으로 fit하고 같은 스케일로 STRU 실선을 Match하므로 긴 외부 Assembly 전체 때문에 STRU가 작아지지 않는다. ISO에는 부모 Assembly 정보를 이름 문맥으로 사용한 `A. Assembly / Part`, Z/X/Y에는 A/A1 접합영역 기호를 실제 접합 중심에 표시한다. 치수는 선택 STRU 전체 범위와 연결 Part 끝단↔접합 영역 체인만 사용하며 연결 Assembly 전체 범위는 제외한다. 상세는 [설치도 연결 영역 Osnap 치수](../치수/설치도%20치수%20추출.md) 참고.
...
## 9. 관련 링크
- 코드 구현: [Form1.DrawingSheets.cs:L1104](../../code-reference/form1-drawing-sheets.md#btnGenerateSheet2D_Click)
