...
| 2 | Z (Looking Z) | `ShowAllDimensions("Z", true, estScale)` → **`MarkNonRightAngles("Z")`** → `shapeDrawingIds` → `Add2DObjectFromShapeDrawing` + `ApplyParallelTextShift` + `Add2DMeasureFrom3DMeasure` | × 0.65 |
...
- **각도 표시 (재설계 중, 2026-06-23 호출 비활성)**: 1차 구현 `MarkNonRightAngles`는 *한 부재 내부*의 모서리 각(ㄱ자 꺾임)을 표시했으나, 사용자 요구는 *부재-부재 접합부가 수직·수평이 아닐 때*의 각도였다 → 호출 비활성, 접합 각도 로직으로 교체 예정. (아래 설명은 폐기될 1차 구현 기준) `ShowAllDimensions` 직후 호출해 같은 `Review.Measure`→2D 파이프라인에 각도를 누적시킨다. 부재별 `GetOsnapPoint`의 LINE 세그먼트를 **3D 꼭짓점**으로 묶어(화면상 겹치지만 실제로 안 만나는 모서리 배제) 화면 평면 2축으로 투영한 사잇각을 구하고, **90°의 배수(90/180/270/360)가 아니면** `AddCustom3PointAngle`로 표시한다. 각도는 정수 도(度)·파랑, 생성 ID에만 `SetStyle(id, …)` 적용(전역 미변경). `ApplyParallelTextShift`는 각도(`RK_MEASURE_ANGLE`)를 시프트 대상에서 제외. ISO 뷰는 호출하지 않음.
