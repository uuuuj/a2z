...
- **부재-부재 접합 각도 표시 (`MarkNonRightAngles`, 2026-06-23)**: `ShowAllDimensions` 직후 호출해 같은 `Review.Measure`→2D 파이프라인에 각도를 누적시킨다. **서로 다른 두 부재가 접합하는 곳**(osnap 끝점 3mm 이내 근접 = 연결, 접합점=최근접 점쌍 중점)에서 두 부재 **길이축**(osnap 최원점쌍 방향)의 **실제 3D 사잇각이 90°의 배수(0/90/180/270/360)가 아니면**(=틀어져 만나면) `AddCustom3PointAngle(접합점, A방향점, B방향점)`로 표시한다. **한 부재 *내부* 모서리 각(ㄱ자 꺾임)은 표시하지 않는다.** 판형(PAD/PLATE)은 길이방향이 모호해 제외. 깊이축에 평행해 화면상 점이 되는 뷰에서는 생략. 연결성·접합점은 osnap 근접으로 자체 판정해 간섭검사(`clashList`) 상태에 의존하지 않는다. 정수 도(度)·파랑, 생성 ID에만 `SetStyle(id, …)`. `ApplyParallelTextShift`는 각도(`RK_MEASURE_ANGLE`)를 시프트 제외. ISO 미호출. ⚠ 실측 확정: AddCustom3PointAngle 인자 순서(예각 모델)·표시값이 3D각인지 투영각인지(코드가 `[각도]` 로그에 둘 다 기록).
