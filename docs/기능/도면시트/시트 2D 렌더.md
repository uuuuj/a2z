...
- **Input 슬롯 선초기화 (2026-07-10)**: `data` dict를 1..159 빈 문자열로 선초기화 — 코드가 안 채우는 슬롯의 `{Input_N}` 태그가 도면에 그대로 노출되는 것 방지 (가공도와 동일 패턴). 신 템플릿 슬롯 확장 대비: 124=Note 본문, 125~128=PAINT CODE/DP No 구역, 129=TAG NO, 130~159=Rev 이력 표(5행×6열).
- **신 템플릿 준비 (2026-07-10, 미전환)**: 사용자 제작 `제작도_도면_1.xlsx`(297×210 그리드, 1칸=1mm, 제도 교본 여백)에 플레이스홀더 삽입 완료 — View_1~4(ISO/Z/X/Y) + View_5(North Arrow)/View_6(CLIENT 로고 예약)/View_7(ISO North Arrow) + `{Image}`(CONTRACTOR 로고) + Input_1~159 (BOM 8열×15행=4~123 기존 컨벤션 유지, 16~20행은 빈 셀). **코드는 아직 기존 `사용자템플릿_엑셀_제작도.xlsx` 참조** — 전환은 경로 한 줄 교체.
