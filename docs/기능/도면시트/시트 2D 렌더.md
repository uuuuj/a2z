...
- **빈 칸 괘선 제거 (2026-07-21)**: `ImportExcelWithData` 직후 `RemoveEmptyTemplateBorders(0.1f, RowAndColumn)`(SDK 1.0.26.716) 호출 — 내용 없는 공백 셀(미기재 BOM 행 등)의 테두리 선을 SDK가 지운다. 전역 동작(BOM만 선별 불가)이라 Rev 표·NOTE 등 다른 빈 칸 괘선도 함께 사라질 수 있음 — 실기 확인 대상.
