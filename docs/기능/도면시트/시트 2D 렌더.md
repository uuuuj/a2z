...
|---|---|---|---|
| 1 | ISO | `CreateIsoBalloonNotes(memberIndices, true)` → `FromScreen` 가시성 필터 → **점선/실선 두 겹 캡처 + Match 정합 (아래 항목)** → `Add2DNoteFrom3DNote(visibleNoteIds)` → 제작도만 Clash 지점에 연결 어셈블리 이름 2D 노트 → 원형 SnapBox | × 0.70 |
