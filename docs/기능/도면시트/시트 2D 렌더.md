...
| 2D 변환 완료 | ISO 실선 투영 객체 선택 → 부재번호 풍선 변환 → 연결 이름 노트 생성 → 선택 해제 → `Set2DViewAlignAreaReviewsPositionByOffset`으로 두 라벨을 View 내부 모델 fit 범위 기준 최종 정렬 |
...
|---|---|---|---|
| 1 | ISO | `CreateIsoBalloonNotes(memberIndices, true)` → `FromScreen` 가시성 필터 → **점선/실선 두 겹 캡처 + Match 정합 (아래 항목)** → ISO 실선 객체 선택 후 `Add2DNoteFrom3DNote(visibleNoteIds)` → 제작도 연결 Assembly 이름 또는 설치도 연결 Part당 `A. Assembly / Part` 이름 1개 생성 → 선택 해제 → 모델 fit 70% 영역 기준 풍선·연결 이름 통합 정렬 → 최종 `Render` | × 0.70 |
...
- **ISO 리뷰 최종 정렬 (SDK 1.0.26.723)**: `Add2DNoteFrom3DNote`로 부재번호 풍선을 변환하고 제작도·설치도 연결 이름 노트까지 생성한 다음, 모든 2D 객체 선택을 해제하고 `Set2DViewAlignAreaReviewsPositionByOffset(rectMin, rectMax, 10f)`를 한 번 호출한다. SDK는 입력 사각형 **바깥쪽**으로 리뷰 라벨을 밀기 때문에 전체 `{View_1}`을 입력하면 템플릿 밖으로 나간다. 따라서 `rectMin/rectMax`는 실제 ISO 모델 배치 중심(`View 중심 + X 10mm, Y 15mm`)과 fit 가능 폭·높이의 70% envelope로 계산한다. 사각형 좌표는 2D 캔버스 mm이고 `offset=10f`만 배포 예제상 **픽셀 기준**이다. 부재번호 풍선과 연결 이름 라벨은 모델 밖·View 안쪽의 같은 영역에 배치되며, 연결 이름의 지시선 Target은 기존 실제 접합점에 유지된다. API 예외는 로그만 남기고 각 노트의 기존 위치로 계속 출력한다.
...
- **ISO 점선 두 겹 표현 (2026-07-21 — 소프트힐스 신 API 3종)**: 조립도(Sheet2+, `BaseMemberIndex ≥ 0`)는 전체 구조를 LONG_DASHED 점선 배경으로 깔고 **기준부재만 실선**(프레임 = 전체 fit), 제작도(Sheet1, `-1`)는 시트 부재 실선 + **간섭검사로 붙은 시트 밖 부재를 점선**. 조립도는 실기 정상인 기존 순서(점선 캡처 → 실선 캡처 → 점선 fit·이동 → `Match(실선, 점선)`)를 유지한다. 제작도는 **시트 부재+연결 부재를 함께 점선 배경으로 캡처** → 시트 부재 노드 기준 CropFit → LONG_DASHED → 점선 fit·이동 → 시트 부재만 실선 캡처 → `Match(실선, 점선)` 순서를 사용한다. 제작도 점선 대상이 없으면 단일 캡처 폴백.
  ...
  - **연결 어셈블리 이름**: 연결 결과의 Part에서 부모 방향으로 올라가 가장 가까운 `NodeKind.ASSEMBLY` 이름을 선택한다. 같은 Assembly의 여러 Part가 닿아도 이름은 한 번만 표시한다. `Add2DNoteFromWorldCoordinate`의 Target은 실제 Clash `HotPoint` X/Y/Z, 초기 Label은 같은 점에서 월드 X축 ±100mm(`IsoNeighborNoteWorldOffset`) 이동한 위치다. 두 겹 객체의 Crop/fit/Match가 모두 끝난 뒤 연결 부재가 포함된 점선 객체만 선택해 투영하고 즉시 선택 해제하며, 최종 Label은 부재번호 풍선과 같은 모델 fit 70% 영역으로 정렬한다. Assembly를 찾지 못하면 Part 이름, 그것도 없으면 Clash 결과 이름으로 폴백한다. 조립도에는 적용하지 않는다.
  - 실기 판정 로그: `제작도 연결 후보 캐시 완료`(Body 수·캐시 시간)·`제작도 연결 후보 선별`(전체/대상/후보 수)·`제작도 연결 간섭검사 결과`(원본/Part 수·이름)·`P2 ISO 두겹`(match·점선 크기)·`P2 ISO 제작도 시트 밖 간섭 이웃 n개`·`P2 ISO 제작도 이웃 점선 CropFit`(대상 노드 수·offset)·`P2 ISO 제작도 연결 이름 노트`(후보/생성 수)·`P2 ISO 리뷰 영역 정렬`(풍선/연결 이름 수·영역·offset).
