...
- **ISO 점선 두 겹 표현 (2026-07-21 — 소프트힐스 신 API 3종)**: 조립도(Sheet2+, `BaseMemberIndex ≥ 0`)는 전체 구조를 LONG_DASHED 점선 배경으로 깔고 **기준부재만 실선**(프레임 = 전체 fit), 제작도(Sheet1, `-1`)는 시트 부재 실선 + **간섭검사로 붙은 시트 밖 부재를 점선**. 조립도는 실기 정상인 기존 순서(점선 캡처 → 실선 캡처 → 점선 fit·이동 → `Match(실선, 점선)`)를 유지한다. 제작도만 점선 캡처 → CropFit → LONG_DASHED → 점선 fit·이동 → 실선 캡처 → `Match(실선, 점선)` 순서를 사용한다. 제작도 점선 대상이 없으면 단일 캡처 폴백.
  ...
  - 실기 판정 로그: `제작도 연결 후보 캐시 완료`(Body 수·캐시 시간)·`제작도 연결 후보 선별`(전체/대상/후보 수)·`제작도 연결 간섭검사 결과`(원본/Part 수·이름)·`P2 ISO 두겹`(match·점선 크기)·`P2 ISO 제작도 시트 밖 간섭 이웃 n개`·`P2 ISO 제작도 이웃 점선 CropFit`(대상 노드 수·offset).
