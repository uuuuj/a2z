...
- **설치도 4개 뷰 두 겹 표현 (2026-07-21)**: 설치도(`BaseMemberIndex == -2`)는 ISO뿐 아니라 Z/X/Y에서도 선택 STRU 전체를 실선, 직접 연결된 외부 Assembly 전체를 LONG_DASHED 점선으로 캡처한다. 점선 캡처에는 선택 STRU도 함께 넣고 그 위에 동일 STRU 실선을 Match하므로, 프레임은 선택·연결 양쪽 전체를 기준으로 맞고 외부 Assembly는 잘리지 않는다. ISO에는 `A. Assembly / Part`, Z/X/Y에는 A/A1 접합영역 기호를 실제 접합 중심에 표시한다. 치수는 [설치도 연결 영역 Osnap 치수](../치수/설치도%20치수%20추출.md)의 사전 계산 결과를 사용한다.
