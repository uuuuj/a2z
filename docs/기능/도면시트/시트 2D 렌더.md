...
| 13 | 풍선 배치 | Form1/SDK | 3D 번호 풍선 생성·가시성 선별 후 ISO 투영 객체에 2D 변환, SDK가 실제 `{View_1}` 영역 기준으로 최종 정렬 |
...
| 2D 변환 완료 | ISO 실선 투영 객체 선택 → `Add2DNoteFrom3DNote` → 선택 해제 → `Set2DViewAlignAreaReviewsPositionByOffset`으로 실제 View 영역 기준 최종 정렬 |
| SDK 최종 정렬 실패 | 경고 로그를 남기고 기존 AABB 배치 결과로 출력 계속 |
...
|---|---|---|---|
| 1 | ISO | `CreateIsoBalloonNotes(memberIndices, true)` → `FromScreen` 가시성 필터 → **점선/실선 두 겹 캡처 + Match 정합 (아래 항목)** → ISO 실선 객체 선택 후 `Add2DNoteFrom3DNote(visibleNoteIds)` → 선택 해제 → View 영역 기준 풍선 정렬 → 제작도 연결 Assembly 이름 또는 설치도 연결 Part당 `A. Assembly / Part` 이름 1개 → 최종 `Render` | × 0.70 |
...
- **ISO 풍선 최종 정렬 (SDK 1.0.26.723)**: `Add2DNoteFrom3DNote` 직후 실선 투영 객체를 선택 해제한 다음 `Set2DViewAlignAreaReviewsPositionByOffset(rectMin, rectMax, 10f)` 호출. `rectMin/rectMax`는 `{View_1}`의 엑셀 원본 좌표에 Core 기본 템플릿 패딩 15mm를 X/Y 모두 더한 2D 캔버스 좌표(mm, Z=0)다. `offset=10f`는 배포 예제상 **픽셀 기준**이며 내부에서 좌표 변환된다. 연결 Assembly/Part 이름 노트는 정렬 호출 뒤에 생성해 부재번호 풍선과 분리한다. API 예외는 로그만 남기고 기존 AABB 배치 결과로 계속 출력한다.
...
| E05 | ISO 풍선 영역 정렬 API 예외 | catch 후 기존 위치 유지 | `[P2 ISO 풍선 영역 정렬 WARN]` 로그 | 나머지 뷰와 PDF 생성 계속 |
...
| 날짜 | 변경 내용 | 작성자 |
|---|---|---|
| 2026-07-23 | SDK 1.0.26.723 배포 예제 순서대로 ISO 실선 투영 객체를 선택해 부재번호 풍선을 2D 변환하고, `{View_1}` 좌표에 템플릿 패딩 15mm를 더한 캔버스 영역을 기준으로 `offset=10px` 최종 정렬. 연결 Assembly/Part 이름은 정렬 후 생성하며 실패 시 기존 AABB 배치로 계속 출력 | Codex |
