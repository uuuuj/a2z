...
|---|---|---|---|
| 1 | ISO | `CreateIsoBalloonNotes(memberIndices, true)` → `FromScreen` 가시성 필터 → **점선/실선 두 겹 캡처 + Match 정합 (아래 항목)** → `Add2DNoteFrom3DNote(visibleNoteIds)` → 원형 SnapBox | × 0.70 |
...
- **2단 텍스트 슬라이드 (2026-07-06 사용자 사양)**: 2단에 그려지는 치수(승격 작은 치수 + 기존 2단 체인)는 텍스트를 치수선 방향으로 **종이 절대 5mm** 일괄 이동 — 가로 치수=화면 오른쪽, 세로 치수=화면 위(SDK가 치수선 방향 성분만 반영해 세로는 오른쪽 불가). 실기 검증된 뷰 매핑(X뷰 right=+Y·up=+Z / Y뷰 right=−X·up=+Z / Z뷰 right=+X·up=+Y). 상수 `Lvl2TextSlideCanvas=5`. 진단 로그 `[TextSlide]`.
...
- **ISO 점선 두 겹 표현 (2026-07-21, issue #7 — 소프트힐스 신 API 3종)**: 조립도(Sheet2+, `BaseMemberIndex ≥ 0`)는 전체 구조를 LONG_DASHED 점선 배경으로 깔고 **기준부재만 실선**(프레임 = 전체 fit), 제작도(Sheet1, `-1`)는 시트 부재 실선 + **간섭검사로 붙은 시트 밖 부재를 점선**(프레임 = 시트 fit). 순서 = 점선 먼저 캡처 → (제작도) **점선을 시트 부재 영역+여백만 남기고 Crop** → 실선 캡처(위에 그려짐) → fit 기준 객체(조립도=점선 배경/제작도=실선) 영역 배치 → 스케일 통일 → `Match2DObjectsTo3DObjectPosition(이동, 기준)` 정합. 점선 대상이 없으면(이웃 0개·부재 1개) 단일 캡처 폴백.
  - **제작도 이웃 Crop**: `CropFit2DViewObjectByNodeIDs(이웃점선객체, 시트부재 3D노드, cropOffset)` — 붙은 부위 주변만 남기고 잘라 "실선 부재가 주변 어디에 붙는지"만 표현. `cropOffset`(여백 비율) = 상수 `IsoNeighborCropOffset`(현 0.5f, 실기 튜닝값 — 0이면 시트 영역에 딱 맞고 클수록 맥락 더 보임). 조립도는 전체를 배경으로 보여줘야 하므로 Crop 안 함.
  - **이웃 산출**: `GetClashNeighborPartsOutsideSheet` — clashList Part 쌍의 한쪽만 시트 소속이면 반대쪽 수집 (간섭검사가 보이는 부재 한정이면 빈 목록 → 로그로 판정, 한정 간섭검사 후속 후보). 풍선은 현행 유지.
  - 실기 판정 로그: `P2 ISO 두겹`(match·점선 크기)·`P2 ISO 제작도 시트 밖 간섭 이웃 n개`·`P2 ISO 제작도 이웃 점선 CropFit`(대상 노드 수·offset).
