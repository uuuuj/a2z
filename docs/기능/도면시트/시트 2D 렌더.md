...
| 2D 변환 완료 | ISO 실선 투영 객체 선택 → `Add2DNoteFrom3DNote` → 선택 해제 → `Set2DViewAlignAreaReviewsPositionByOffset`으로 View 내부 모델 fit 범위 기준 최종 정렬 |
...
|---|---|---|---|
| 1 | ISO | `CreateIsoBalloonNotes(memberIndices, true)` → `FromScreen` 가시성 필터 → **점선/실선 두 겹 캡처 + Match 정합 (아래 항목)** → ISO 실선 객체 선택 후 `Add2DNoteFrom3DNote(visibleNoteIds)` → 선택 해제 → 모델 fit 70% 영역 기준 풍선 정렬 → 제작도 연결 Assembly 이름 또는 설치도 연결 Part당 `A. Assembly / Part` 이름 1개 → 최종 `Render` | × 0.70 |
...
- **ISO 풍선 최종 정렬 (SDK 1.0.26.723)**: `Add2DNoteFrom3DNote` 직후 실선 투영 객체를 선택 해제한 다음 `Set2DViewAlignAreaReviewsPositionByOffset(rectMin, rectMax, 10f)` 호출. SDK는 입력 사각형 **바깥쪽**으로 풍선을 밀기 때문에 전체 `{View_1}`을 입력하면 템플릿 밖으로 나간다. 따라서 `rectMin/rectMax`는 실제 ISO 모델 배치 중심(`View 중심 + X 10mm, Y 15mm`)과 fit 가능 폭·높이의 70% envelope로 계산한다. 사각형 좌표는 2D 캔버스 mm이고 `offset=10f`만 배포 예제상 **픽셀 기준**이다. 결과적으로 풍선은 모델 밖이면서 View 안쪽 여백에 남는다. 연결 Assembly/Part 이름 노트는 정렬 호출 뒤에 생성해 부재번호 풍선과 분리한다. API 예외는 로그만 남기고 기존 AABB 배치 결과로 계속 출력한다.
