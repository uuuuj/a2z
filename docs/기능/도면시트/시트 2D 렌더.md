...
- **ISO 점선 두 겹 표현 (2026-07-21, issue #7 — 소프트힐스 신 API)**: 조립도(Sheet2+, `BaseMemberIndex ≥ 0`)는 전체 구조를 LONG_DASHED 점선 배경으로 깔고 **기준부재만 실선**(프레임 = 전체 fit), 제작도(Sheet1, `-1`)는 시트 부재 실선 + **간섭검사로 붙은 시트 밖 부재를 점선**(프레임 = 시트 fit, 이웃 잘림 허용). 순서 = 점선 먼저 캡처 → 실선 캡처(위에 그려짐) → fit 기준 객체(조립도=점선 배경/제작도=실선) 영역 배치 → 스케일 통일 → `Match2DObjectsTo3DObjectPosition(이동, 기준)` 정합. 점선 대상이 없으면(이웃 0개·부재 1개) 단일 캡처 폴백. 제작도 이웃은 `GetClashNeighborPartsOutsideSheet` — clashList Part 쌍의 한쪽만 시트 소속이면 반대쪽 수집 (간섭검사가 보이는 부재 한정이면 빈 목록 → 로그로 판정, 한정 간섭검사 후속 후보). 풍선은 현행 유지. 실기 판정 로그: `P2 ISO 두겹`(match·점선 크기)·`P2 ISO 제작도 시트 밖 간섭 이웃 n개`.
...
| 2026-07-21 | **ISO 점선 두 겹 표현 — `Match2DObjectsTo3DObjectPosition` 적용** (issue #7, 소프트힐스 예제 구조). 조립도: 전체−기준 LONG_DASHED 배경 + 기준부재만 실선(전체 fit 프레임) / 제작도: 시트 실선 + 간섭 시트 밖 이웃 점선(시트 fit·잘림 허용, 0개면 단일 캡처 폴백 + `GetClashNeighborPartsOutsideSheet` 신설). 엑셀 본진(P2) ISO 분기에 두 겹 캡처 신설, 옛 그리드 경로의 T-013 수동 정렬(WorldToScreen·8꼭지점 보정 133줄)은 Match 호출 7줄로 대체 + DASHED_DOUBLEDOTTED→LONG_DASHED·전경을 기준부재만으로 통일. 캡처는 검증된 은선판 유지(예제의 은선 없는 캡처판은 크래시 조합이라 회피). 사내 실기 검증 대기 | Claude |
