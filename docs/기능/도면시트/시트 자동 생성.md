...
last_updated: 2026-07-22 (설치도 직접 연결 Part·접합 영역 사전 준비)
...
| 9.5 | **시트별 치수 사전 준비** | Form1 | Sheet 1은 메인 치수 추출 결과를 그대로 저장. 일반 시트는 부재별 Osnap 맵을 재사용해 계산. 설치도는 선택 STRU 전체 범위와 실제 접촉 Target Body의 가까운 끝단→Connected Body 접합측 모서리 위치 치수를 준비한다. 같은 Body 쌍 A1/A2는 하나로 병합하고 연결 Assembly 전체 범위는 제외. 가공도 3D 장면은 제외 |
