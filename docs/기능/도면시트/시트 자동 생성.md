...
last_updated: 2026-07-21 (도면 리스트 표시 전 시트 조회 데이터 사전 준비)
...
| 9.5 | **시트별 치수 사전 준비** | Form1 | Sheet 1은 메인 치수 추출 결과를 그대로 저장. 일반 시트는 부재별 Osnap 맵을 재사용해 계산하고, 설치도는 UI 상태를 바꾸지 않는 BBox 계산으로 준비. 같은 부재 구성·같은 엔진 결과는 공유. 가공도 3D 장면은 제외 |
...
| 시트 준비 데이터 | 없음 | 일반·설치 시트 `PreparedDimensions`, 모든 시트 `PreparedBomRows`·`PreparedBomNodeGroupMap` |
