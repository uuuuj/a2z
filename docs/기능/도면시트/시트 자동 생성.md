...
| 9.5 | **시트별 치수 사전 준비** | Form1 | Sheet 1은 메인 치수 추출 결과를 그대로 저장. 일반 시트는 부재별 Osnap 맵을 재사용해 계산. 설치도는 선택 STRU의 LINE/POINT Osnap 전체 범위와 `연결 Part MIN → 접합 영역 → Part MAX` 위치 치수를 준비하고 연결 Assembly 전체 범위는 제외. 가공도 3D 장면은 제외 |
