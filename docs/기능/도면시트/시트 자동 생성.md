...
## 1. 개요
Clash 결과를 인접 리스트로 간주하고, 각 부재를 중심으로 **1-hop 이웃**(직접 Clash로 연결된 부재)을 묶어 시트를 만든다. 시트 구성을 확정한 뒤 일반·설치 시트의 치수와 모든 시트의 BOM 행을 먼저 준비하며, 준비가 끝난 후에만 도면 리스트를 표시한다.
...
flowchart TD
    ...
    H --> I[설치도 Sheet<br/>직접 연결 Part·접합 영역 준비]
    I --> J[가공도 Sheet<br/>각 부재마다 1개]
    J --> K[Sheet 1과 동일 구성 제거]
    K --> K2[SheetNumber 1부터 재채번<br/>T-053]
    K2 --> K3[모든 시트 치수·BOM 사전 준비]
    K3 --> L[준비 완료 후 lvDrawingSheet 갱신]
```
...
| 7 | 설치도 Sheet | Form1 | 선택 STRU의 전체 BOM을 실선 대상으로 저장하고, 외부 연결 Clash Part 쌍에서 실제 BODY 접합선/Mesh 영역과 직접 연결된 외부 Part를 점선 대상으로 사전 준비. 부모 Assembly는 이름 표시에만 보존. `BaseMemberIndex = -2`, `BaseMemberName = "설치도"` |
...
| 9.5 | **시트별 치수 사전 준비** | Form1 | Sheet 1은 메인 치수 추출 결과를 그대로 저장. 일반 시트는 부재별 Osnap 맵을 재사용해 계산. 설치도는 실제 접촉 Target Body의 가까운 끝단→Connected Body 접합측 모서리 위치 치수만 준비한다. 같은 Body 쌍 A1/A2는 하나로 병합하고 선택 STRU·연결 Assembly 전체 범위는 제외. 가공도 3D 장면은 제외 |
| 9.6 | **시트별 BOM 사전 준비** | Form1 | 모델 로드 때 만든 `bodyToPartIndexMap`을 재사용하고 관련 Part의 5개 UDA를 Part별 한 번만 읽음. 이후 각 시트의 BOM 행·Body 그룹 맵은 메모리 필터링으로 생성. Sheet 1 결과는 즉시 BOM 표에 적용 |
| 10 | 준비 완료 후 ListView 갱신 | UI | 모든 사전 준비가 끝난 뒤 SheetNumber / 기준부재 / 포함부재(item 번호 콤마) / 부재 수를 표시. **item 번호 = `bomList` 순서(i+1) = ISO 풍선 번호 = BOM 정보 탭 No.** |
...
| 시트 준비 데이터 | 없음 | 일반·설치 시트 `PreparedDimensions`, 설치 시트 `InstallationConnections`·`InstallationContextIndices`, 모든 시트 `PreparedBomRows`·`PreparedBomNodeGroupMap` |
...
## 8. 후행 기능 (Chained)
- [시트 선택 시 X-Ray 표시](./시트%20선택.md)
- [시트별 2D 생성](./시트%202D%20렌더.md)
...
| 날짜 | 변경 내용 | 작성자 |
|---|---|---|
| 2026-07-23 | 설치도 사전 치수에서 선택 STRU 전체 범위를 제거하고 Target Body 끝단→Connected Body 모서리 연결 거리만 준비 | Codex |
| 2026-07-22 | 설치도 사전 치수를 접합영역 체인에서 실제 접촉 Target Body 끝단→Connected Body 모서리 위치 치수로 교체하고 같은 Body 쌍 영역을 병합 | Codex |
| 2026-07-22 | 설치도 사전 표시 대상을 직접 연결 Part로 축소하고, 선택 STRU 전체 범위·연결 Part 접합 치수만 준비하도록 변경 | Codex |
| 2026-07-21 | 설치도 생성 시 선택 STRU에 직접 연결된 외부 Assembly 전체와 BODY 접합선/Mesh 기반 접합 영역을 저장하고, Osnap 전체 범위·접합 위치 치수를 목록 표시 전에 준비하도록 변경 | Codex |
| 2026-07-21 | 도면 리스트 표시 전에 일반·설치 시트 치수와 모든 시트 BOM을 사전 준비하도록 변경. Sheet 1 치수는 기존 계산 결과 재사용, Body→Part 매핑은 모델 로드 캐시 재사용, UDA는 관련 Part별 1회 조회. 준비 완료 후 목록을 표시해 첫 선택부터 재계산 없이 조회 | Codex |
