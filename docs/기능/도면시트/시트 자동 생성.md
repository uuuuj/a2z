...
last_updated: 2026-07-21 (설치도 외부 연결 Assembly·접합 영역 사전 준비)
...
flowchart TD
    ...
    H --> I[설치도 Sheet<br/>외부 연결 Assembly·접합 영역 준비]
...
| 7 | 설치도 Sheet | Form1 | 선택 STRU의 전체 BOM을 실선 대상으로 저장하고, 외부 연결 Clash Part 쌍에서 실제 BODY 접합선/Mesh 영역과 직접 연결된 외부 Assembly 전체를 사전 준비. `BaseMemberIndex = -2`, `BaseMemberName = "설치도"` |
...
| 9.5 | **시트별 치수 사전 준비** | Form1 | Sheet 1은 메인 치수 추출 결과를 그대로 저장. 일반 시트는 부재별 Osnap 맵을 재사용해 계산. 설치도는 선택 STRU·연결 Assembly의 LINE/POINT Osnap 전체 범위와 `연결 Part MIN → 접합 영역 → Part MAX` 위치 치수를 준비. 가공도 3D 장면은 제외 |
