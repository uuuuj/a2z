...
## 1. 개요
선택 시트를 Y축 방향(XZ 평면)으로 표시한다. 설치도는 선택 STRU+외부 연결 Assembly 전체와 Z 주축·X 보조축의 전체 범위·접합 위치 치수를 표시한다. 3D 미리보기와 2D 도면은 같은 시트 치수 목록을 사용하며, 공통 가시성·치수 흐름은 [X축 뷰](./X축%20도면.md)와 동일하다.
