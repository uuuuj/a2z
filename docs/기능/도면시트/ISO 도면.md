...
## 1. 개요
선택된 도면 시트를 ISO 방향으로 보여주고 ISO 전용 풍선 노트(`CreateIsoBalloonNotes`)만 생성한다. 설치도 3D 미리보기는 선택 STRU와 직접 연결된 외부 Part를 함께 표시한다. 설치도 접합 영역 치수 데이터는 X/Y/Z와 2D 도면용으로 유지하지만 ISO 3D 화면에는 그리지 않는다. PDF의 ISO 뷰는 선택 STRU를 기준으로 맞추고 연결 Part만 점선으로 남긴다.
...
| 설치도 | 선택 STRU와 직접 연결된 외부 Part를 표시하고 접합 영역 치수 데이터만 준비. ISO 화면은 선택 STRU 풍선만 표시 |
...
| `chainDimensionList` | 이전 | 설치도일 때 선택 STRU 전체 범위·연결 Part 접합 영역 치수 적용 |
