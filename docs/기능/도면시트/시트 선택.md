...
last_updated: 2026-07-22 (설치도 직접 연결 Part 표시)
...
| `sheet.BaseMemberIndex == -2` (설치도) | 선택 STRU와 직접 연결된 외부 Part만 표시하고, 목록 생성 때 선택 STRU 전체 범위·실제 접합 영역 기준으로 계산한 `PreparedDimensions`를 적용. 연결 Assembly 전체 범위 치수는 생성하지 않는다. 예외적인 미준비 시트만 `ExtractInstallationDimensions(sheet)` fallback |
...
## 9. 관련 링크
- 코드 구현: [Form1.DrawingSheets.cs:L571](../../code-reference/form1-drawing-sheets.md#LvDrawingSheet_SelectedIndexChanged)
