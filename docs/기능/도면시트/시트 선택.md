...
| `sheet.BaseMemberIndex == -2` (설치도) | 선택 STRU와 직접 연결된 외부 Part만 표시하고, 목록 생성 때 선택 STRU 전체 범위·Target Body 가까운 끝단→Connected Body 접합측 모서리 기준으로 계산한 `PreparedDimensions`를 적용. 접합 중심·A1/A2·연결 Assembly 전체 범위 치수는 생성하지 않는다. 예외적인 미준비 시트만 `ExtractInstallationDimensions(sheet)` fallback |
