...
last_updated: 2026-07-21 (설치도 외부 연결 Assembly 표시)
...
## 1. 개요
`lvDrawingSheet` 선택이 변경되면 해당 시트 부재만 표시한다. 단, 설치도는 선택 STRU와 직접 연결된 외부 Assembly 전체를 함께 표시한다. 일반 시트와 설치도는 목록 생성 전에 준비한 치수·BOM 데이터를 즉시 적용하고, 가공도만 부재별 3D 장면 생성을 수행한다.
...
flowchart TD
    ...
    B --> C[X-Ray 비활성화]
    C --> D[전체 부재 Hide]
    D --> E[시트 표시 대상 Show<br/>설치도=STRU+외부 연결 Assembly]
...
| 4 | 전체 부재 Hide | SDK | `Object3DKind.ALL`을 숨겨 이전 설치도의 외부 Assembly 잔존까지 제거 |
...
| `sheet.BaseMemberIndex == -2` (설치도) | 선택 STRU와 직접 연결된 외부 Assembly 전체를 표시하고, 목록 생성 때 실제 접합 영역·Osnap 기준으로 계산한 `PreparedDimensions`를 적용. 예외적인 미준비 시트만 `ExtractInstallationDimensions(sheet)` fallback |
...
| 대상 | Before | After |
|---|---|---|
| 부재 가시성 | 전체 또는 이전 | 일반=시트 부재, 설치도=선택 STRU+외부 연결 Assembly |
