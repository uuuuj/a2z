...
## 1. 개요
`lvDrawingSheet` 선택이 변경되면 해당 시트 부재만 표시한다. 단, 설치도는 선택 STRU와 **연결이 확인된 외부 서포트 STRU 전체**를 함께 표시한다 (#63). 화면 맞춤은 선택 STRU 기준이라 연결 서포트가 커도 도면이 작아지지 않고, 카메라에 안 들어오는 부분은 잘린다. Assembly 정보는 접합 이름 문맥으로만 유지한다. 일반 시트와 설치도는 목록 생성 전에 준비한 치수·BOM 데이터를 즉시 적용하고, 가공도만 부재별 3D 장면 생성을 수행한다.
...
## 3. 사전 조건
- [ ] 도면 시트 목록 채워짐 ([SHT-001](./시트%20자동%20생성.md))

## 4. 전체 동작 흐름 (Happy Path)

```mermaid
flowchart TD
    A[시트 선택 변경] --> A1{"가공도가 아닌 시트인가"}
    A1 -- 예 --> A2["가공도 미리보기 참조축·화면 회전 원복"]
    A1 -- 아니오 --> B
    A2 --> B[BeginUpdate]
    B --> C[X-Ray 비활성화]
    C --> D[전체 부재 Hide]
    D --> E[시트 표시 대상 Show<br/>설치도=선택 STRU+연결 서포트 STRU 전체]
...
| 2 | 가공도 카메라 상태 정리 | Form1 | 가공도가 아닌 시트로 이동할 때 미리보기의 화면 회전과 활성 ReferenceAxis를 원복·삭제 |
| 3 | BeginUpdate | SDK | — |
| 4 | X-Ray 해제 | SDK | `XRay.Enable = false` + Clear |
| 5 | 전체 부재 Hide | SDK | `Object3DKind.ALL`을 숨겨 이전 설치도의 외부 연결 부재 잔존까지 제거 |
| 6 | 시트 표시 대상 Show | SDK | 일반 시트는 `MemberIndices`, 설치도는 `MemberIndices + InstallationContextIndices`(연결 서포트 STRU 전체) |
| 6-1 | 카메라 맞춤 | SDK | 일반 시트는 표시 대상 전체 기준, **설치도는 `MemberIndices`(선택 STRU) 기준** — 연결 서포트까지 넣으면 선택 STRU가 작아진다 (#63) |
| 7 | 실루엣 유지 | SDK | Green |
| 8 | xray 인덱스 저장 | Form1 | 글로벌 뷰 버튼용 |
| 9 | 카메라 이동 | SDK | `View.EnableAnimation = false` 후 `FlyToObject3d(MemberIndices, 1.2f)` — 중간 이동 화면 없이 즉시 fit |
| 10 | 심볼/풍선 제거 | SDK | `Clash.ClearResultSymbol()`, `Review.Note.Clear()` |
| 11 | **기준부재 선택상태** (T-022) | SDK | `Object3D.Select(DESELECT_ALL)` → 기준부재 하나만 `Select(indices, true, false)` → 3D View에서 빨간색 하이라이트. Sheet 1(-1)·설치도(-2)는 생략, 가공도(-3)는 `MemberIndices[0]`, Sheet 2+는 `BaseMemberIndex` |
| 12 | EndUpdate | SDK | — |
| 13 | 치수 분기 | Form1 | [분기 A]. 일반·설치 시트는 `PreparedDimensions` 적용만 수행 |
| 14 | BOM 정보 적용 | Form1 | `PreparedBomRows`와 `PreparedBomNodeGroupMap`을 ListView·공유 맵에 복사. 전체 모델/SDK 재조회 없음 |
| 15 | 구간별 성능 로그 | Form1 | 장면 전환·치수 적용·BOM 적용·전체 시간을 각각 기록 |
...
| `sheet.BaseMemberIndex == -2` (설치도) | 선택 STRU와 연결 서포트 STRU 전체를 표시하고, 목록 생성 때 Target Body 가까운 끝단→Connected Body 접합측 모서리 기준으로 계산한 `PreparedDimensions`만 적용. 선택 STRU·연결 Assembly 전체 범위와 접합 중심·A1/A2 치수는 생성하지 않는다. 예외적인 미준비 시트만 `ExtractInstallationDimensions(sheet)` fallback |
...
| 대상 | Before | After |
|---|---|---|
| 부재 가시성 | 전체 또는 이전 | 일반=시트 부재, 설치도=선택 STRU+연결 서포트 STRU 전체 |
...
| 날짜 | 변경 내용 | 작성자 |
|---|---|---|
| 2026-07-23 | 기울어진 가공도 3D 미리보기에서 다른 종류의 시트로 이동할 때 로컬 ReferenceAxis와 화면 회전을 먼저 원복해 다음 시트 카메라에 누적되지 않게 함 | Codex |
| 2026-08-05 | #63: 설치도 표시 대상을 접합 Part 하나에서 그 Part가 속한 **서포트 STRU 전체**로 확대. 카메라 맞춤·치수 baseline은 선택 STRU 고정으로 유지해 표시 범위가 커져도 도면이 축소되지 않는다 | Claude |
