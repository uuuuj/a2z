...
last_updated: 2026-07-23 (홀·슬롯 최종 화면 하단 배치)
...
이 버튼은 **수동 단일 출력**이다. **도면 일괄 출력**(`btnExtractDrawingList` → STRU별 `ProcessSingleStruFull`)에서도 같은 `GenerateMfgDrawingManual`을 호출해 제작/조립/설치도와 함께 가공도를 생성한다. 일괄에서는 저장 위치가 각 STRU 하위 폴더이고, 결과 PDF 수가 STRU 성공 카운트에 합산된다. 일괄 출력의 취소 요청 콜백이 전달된 경우 각 가공도 페이지 시작 전에 요청을 확인해 완료된 페이지 PDF는 유지하고 다음 페이지부터 중단한다. 수동 가공도 출력은 콜백을 전달하지 않아 기존 흐름과 동일하다.
...
엑셀 템플릿을 가져온 직후 `RemoveEmptyTemplateBorders`가 내용 없는 공백 셀(미기재 BOM 행 등)의 괘선을 지운다. 전역 동작이라 남길 칸(PAINT/DP/TAG 165~169·REV 표 첫 기재행 194~199 = REV./DATE/DESC/DRAWN/CHECKED/APPROVED)만 공백(`" "`)으로 위장 보호하고, BOM(1~20행=4~163·21~25행=201~240)·Note(164)·Rev 위 4행(170~193)·부재명(241~245)은 빈칸으로 둬 괘선이 제거되게 한다. 부재명은 REV 표(195~199)와 번호가 겹쳐 REV 칸으로 새던 것을 `Input_241~245`로 분리했고, REV 표 첫 기재행은 제작도와 동일하게 194~199로 복원했다(#32).
...
5. 임시 캡처로 실제 투영 크기를 재어, 세로로 잡히면 화면축 90도 회전으로 가로로 눕힌다(`ProbeAndRollLandscape`). Hole/SlotHole 노트는 이 회전 전에 그리지 않고 대기시킨 뒤, ORIENTATION 참조축과 추가 가로화 회전을 합친 최종 화면 수평·수직축으로 모델 하단을 다시 계산해 생성한다(`AddMfgPendingHoleNotes`). 따라서 회전 전 아래쪽이 최종 화면 왼쪽으로 돌아가는 일이 없다.
...
9. 최종 화면 하단에 만든 Hole/SlotHole 노트, EarthBoss 풍선과 단면 모델(은선 없음 — 은선 캡처 API + 은선제거(HLR) 렌더모드 조합)을 View 영역 중앙에 맞춰 2D로 변환한다.
...
- 최장축 치수는 두 번째 뷰에 맡기고, 나머지 축 치수와 EarthBoss 풍선을 표시한다. Hole/SlotHole은 첫 번째 뷰의 최종 가로 전환이 끝난 뒤 화면 하단에 생성한다.
...
도면번호 목록에서 여는 3D 미리보기는 공통 장면 생성 직후 Review Note를 지우고 대기 중인 Hole/SlotHole 노트를 생성하지 않는다. PDF 행 렌더링만 가로 전환이 끝난 뒤 대기 노트를 최종 화면 하단에 생성하므로 세 형상 풍선의 2D 변환과 PDF 출력은 유지된다.
...
- `BuildMfgSceneCore`: 기본 카메라, Osnap, 치수·Hole/SlotHole 대기 정보 수집, EarthBoss 풍선 생성, 두 뷰 스왑 판정. Osnap은 도면 리스트 뽑기 때 채운 부재별 캐시를 우선 사용(hit면 `GetOsnapPoint` ~0.8초 생략, miss면 직접 추출 폴백 — 2026-07-22). 캐시도 동일 규칙(LINE 양끝+POINT, CIRCLE 제외)이라 결과 동일
...
- `AddMfgPendingHoleNotes`: ORIENTATION 참조축과 화면축 회전을 반영한 최종 화면 기저로 Hole/SlotHole 노트를 모델 하단에 생성
- `DrawMfgDimsAtScale`: 캡처 후 실측 배율로 보조선·치수를 그린다. 기울어진 부재는 ORIENTATION 기반 로컬 축의 UserAxis 측정을 우선하고 정상 부재와 실패 폴백은 기존 월드축 측정을 유지한다
