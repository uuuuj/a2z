...
flowchart TD
    ...
    E --> E1["첫 부재에서 STRU PNT UDA 1회 조회"]
    E1 --> F["5개씩 페이지 분할"]
    F --> G["엑셀 템플릿 가져오기"]
    G --> H["각 행의 부재 렌더"]
    H --> H1["LINE Osnap 주축 판정 로그"]
    H1 --> I{"EA 부재인가"}
    I -- 아니오 --> J["단일 뷰 캡처"]
...
PAINT CODE는 가공도 출력 시작 시 첫 부재에서 부모 방향으로 최대 10단계를 탐색해 키 이름에 `PNT`가 포함된 STRU UDA의 첫 비어 있지 않은 값을 한 번만 조회한다. 조회 결과는 모든 페이지의 `{Input_166}`에 재사용하며, 복수 후보 키·값은 `[PAINT CODE]` 로그에 남긴다. 값이 없으면 `Input_166`의 초기 공백을 유지해 괘선을 보존한다.
...
1. 대상 부재만 표시한다.
2. LINE Osnap을 5도 방향군으로 묶어 길이 합 주축을 판정하고 `[참조축판정]` 로그를 남긴다. 월드축 편차 1도 초과를 `틀어짐`으로 분류하지만 이 단계에서는 카메라·치수·출력을 바꾸지 않는다. 단일 최장선 결과와 `ORIENTATION`도 비교용으로 함께 기록한다.
3. BBox 최장축과 PAD/PLATE 여부로 기본 카메라를 정한다.
4. `ORIENTATION` UDA 회전을 적용한다.
...
8. 모델 캡처 후 확정된 실측 배율로 보조선·치수를 그린다(`DrawMfgDimsAtScale`) — 보조선 오프셋(9/18mm)·시작 gap(2mm) 모두 종이 기준 일정, 2단 치수 텍스트는 치수선 방향으로 종이 2.5mm 이동한다.
...
- `DetectMfgAxis` / `LogMfgAxisDetection`: LINE Osnap 방향군의 길이 합으로 주축을 판정하고 월드축 편차·단일 최장선·ORIENTATION 비교 로그를 남긴다. 참조축은 아직 활성화하지 않는다
