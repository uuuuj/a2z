...
last_updated: 2026-07-23 (일괄 출력 가공도 포함·부재명 241~245)
...
이 버튼은 **수동 단일 출력**이다. **도면 일괄 출력**(`btnExtractDrawingList` → STRU별 `ProcessSingleStruFull`)에서도 같은 `GenerateMfgDrawingManual`을 호출해 제작/조립/설치도와 함께 가공도를 생성한다(2026-07-23, #35). 일괄에서는 저장 위치가 각 STRU 하위 폴더이고, 결과 PDF 수가 STRU 성공 카운트에 합산된다.
