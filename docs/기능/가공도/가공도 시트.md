...
7. 모델 캡처 후 확정된 실측 배율로 보조선·치수를 그린다(`DrawMfgDimsAtScale`) — 보조선 오프셋(6/12mm)·시작 gap(2mm) 모두 종이 기준 일정, 치수 텍스트는 SDK 자동 정렬.
8. Hole, SlotHole, EarthBoss 풍선과 단면 모델(은선 없음)을 View 영역 중앙에 맞춰 2D로 변환한다.
