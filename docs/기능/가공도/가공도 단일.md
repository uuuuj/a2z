...
- 가공도 전용 규칙(`FilterMfgDimensions`)으로 점을 선별한다 — 카메라가 보는 평면의 2축 치수를 추린다. 현재는 제작도 알고리즘(`ApplySmartFiltering`)을 가공도 전용 파라미터(축당 최대 8개, 텍스트 겹침 25mm 회피)로 차용하며, 겹치는 치수는 2단 분배·전체 치수는 항상 표시한다. **규칙·파라미터는 제작도와 독립**이라 가공도 고유 규칙을 따로 발전시킬 수 있다. (이전에는 선별 없이 병합 후 모든 Osnap 점을 뽑았다.)
