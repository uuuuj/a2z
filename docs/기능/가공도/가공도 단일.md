---
feature_id: MFG-001
feature_name: 선택 부재 가공도 생성 (DEPRECATED)
category: MfgDrawing
trigger_type: User Action
owner_module: Form1.MfgDrawing.cs
last_updated: 2026-05-23 (P1 폐기 — btnMfgDrawing 작은 버튼 + 핸들러 제거)
code_reference: /docs/code-reference/form1-mfg-drawing.md#btnMfgDrawing_Click
---

> ⚠️ **DEPRECATED — 2026-05-23 P1에서 폐기**
> 작업/데이터 탭의 작은 "가공도 출력" 버튼(`btnMfgDrawing`)과 그 클릭 핸들러(`btnMfgDrawing_Click`)가 제거됨.
> 가공도는 도면정보 탭의 큰 "가공도 출력" 버튼(`btnMfgDrawingSheet`)으로 통합 진행됨 (계획서 `docs/리팩토링/가공도-수동우선-재배선.md` v3 참조).
> `ExecuteMfgDrawing` 함수 본체는 `LvDrawingSheet_SelectedIndexChanged`(시트 선택 미리보기)에서 그대로 사용 — 함수 자체는 유지.
> 아래 흐름은 폐기된 트리거 기준 옛 내용이라 참고용으로만 보존.

# 선택 부재 가공도 생성 (DEPRECATED)
...
> 구현 상세는 [코드 레퍼런스](../../code-reference/form1-mfg-drawing.md#ExecuteMfgDrawing) 참고
...
## 8. 후행 기능 (Chained)
- [PDF 내보내기](../2D도면/PDF 출력.md)
- [전체 배치 가공도](./가공도 시트.md)
- BOM 더블클릭 → [전체 복원](../2D도면/BOM 목록 더블클릭.md)

## 9. 관련 링크
- 코드 구현: [Form1.MfgDrawing.cs:L19](../../code-reference/form1-mfg-drawing.md#btnMfgDrawing_Click), [ExecuteMfgDrawing](../../code-reference/form1-mfg-drawing.md#ExecuteMfgDrawing)
...
| 2026-04-23 | T-036 재해석·원복: 사용자 재보고 "45도 대각 ISO 뷰 느낌"은 Z 최장축 세로 문제가 아닌 **카메라 방향 자체가 ISO로 잔존**하는 문제였음. 원인은 [LvDrawingSheet_SelectedIndexChanged](../도면시트/시트 선택.md) 공통부의 `FlyToObject3d` 호출이 이전 카메라 방향(예: 글로벌 ISO 누적)을 유지한 채 이동만 하는 것. 가공도 시트 분기 앞에서 `FlyToObject3d` 스킵으로 해결(상세는 시트 선택.md T-036 이력). 본 함수의 **L215 180° 스킵 가드는 원복** — ISO 원인과 무관하며 수직 뒤집기 기능이 본래 의도였음. `use1803d`는 여전히 블록 바깥 스코프로 승격 유지 (DiagLog 가시성) | Claude |
...
| 2026-05-02 | **T-046**: 가공도 보조선 LineType을 `DASHED_DOUBLEDOTTED`(이중쇄선) → `SOLID`(가는 실선)로 변경 — 회사 doc "긴급상 10" + 사용자 확장 ("모든 보조선"). 적용 위치: 메인 보조선([Form1.MfgDrawing.cs:1542](../../../A2Z/Form1.MfgDrawing.cs)) + EA 두 번째 뷰 보조선([Form1.MfgDrawing.cs:1900](../../../A2Z/Form1.MfgDrawing.cs)). 토글 패턴(`SOLID`로 복원하던 호출)도 단일 SOLID 호출로 단순화. **추가**: 보조선 시작점이 모델 표면에서 1mm 떨어지게 gap 적용 — `DrawDimension` 단일 지점 변경으로 가공도(메인/본문/EA) + 일반 시트 + 글로벌 X/Y/Z + 치수추출 4경로 자동 적용. 통합 사양: [치수 보조선 사양.md](../../기술 노트/치수 보조선 사양.md). gap 값(`ExtensionLineGap = 1.0f`)은 [Form1.Dimensions.cs](../../../A2Z/Form1.Dimensions.cs) 한 줄로 조정 가능 | Claude |
| 2026-05-06 | **T-058**: 가공도 3곳(`Form1.MfgDrawing.cs:325` 메인 / `:1050` sub / `:1703` EA) `AlignDistanceTextPosition = 0 → 2` (보조선 바깥 배치). 회사 doc "개발 요청 — 상 5" 사양. 좁은 치수에서 텍스트가 보조선 사이를 침범하는 문제 회피. 통합 사양: [치수 텍스트 위치.md](../../기술 노트/치수 텍스트 위치.md) | Claude |
| 2026-05-14 | **T-064 (도면리스트 뽑기 검증)**: 사용자 사양 *"가공도 보조선 전부 다 제작도랑 똑같이 맞춰줘"* — 가공도 3경로 보조선 길이 일괄 50% 축소. 메인(`L361~377`) `mfgChainOff1 100→50` / `mfgChainOff2 200→100` / `mfgTotalOff 250→125 (≤1000mm)` / `mfgTotalOff 300→150 (>1000mm)`. MULTI 분기(`L1275~1276`)도 동일. EA 신규뷰(`L1929~1930` + `eaTotalOff L1926`)도 동일. `offFactor`(작은 모델 0.5x 축소)는 그대로 적용 — 작은 부재는 추가로 절반. **근사 통일**: 제작도(`ShowAllDimensions` v10)는 캔버스 절대 5/10mm 고정, 가공도는 모델 좌표 기준이라 셀 estScale 의존이 정확함 — 1차 50% 축소로 시각적 근사, 부족 시 estScale 환산 도입 예정 | Claude |
| 2026-05-14 | **T-064 (도면리스트 뽑기 검증 2차)** — ExecuteMfgDrawing 3종 변경: (1) **홀 Osnap 제외** — `osnap.Kind == CIRCLE` 케이스를 별도 분리해 `break` (POINT/LINE만 치수 추출 사용). 홀이 많은 부재의 치수 폭주 회피 — 사용자 사양 *"홀이 있는 부재가 있으면 홀 주변의 Osnap이 모두 치수재는대 사용돼서 엄청 많은 치수가 생기는데"*. (2) **3D 미리보기 풍선 비활성화** — 슬롯홀 풍선 블록 끝 `L548 직후` `vizcore3d.Review.Note.Clear()` 1줄 추가 — 반지름·홀·슬롯홀 풍선이 모두 3D 생성 직후 클리어 → 시트 선택 미리보기·PDF 모두 풍선 없음. 풍선 복귀 시 해당 Clear() 1줄만 주석. (3) 사용자 사양 *"가공도에서 풍선 빼주고 텍스트 키워줘"* 일관 처리 | Claude |
