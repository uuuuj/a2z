...
실기 모델 4건에서 LINE Osnap 기하 판정은 1건만 맞고 `ORIENTATION` UDA는 4건 모두 맞아, 최종 판정은 ORIENTATION 우선으로 확정했다.

1. `ORIENTATION` 원문을 읽어 축과 각도를 파싱한다.
2. 파싱 성공 시 각도가 1도 이하면 `정상`, 1도 초과면 `틀어짐`으로 최종 판정한다.
3. ORIENTATION이 비었거나 파싱되지 않을 때만 대상 부재의 LINE Osnap을 5도 방향군으로 묶고 길이 합 주축과 월드축 편차로 폴백 판정한다.
4. 최종 판정 출처, ORIENTATION 원문·해석값, 기하 결과와 단일 최장 LINE 결과를 함께 로그에 기록한다.

로그 머리말은 `[참조축판정]`이다. 전체 Osnap 수집 때 이미 받은 원본을 판정 캐시에 함께 저장하므로, 일반 경로에서는 부재를 선택할 때 `GetOsnapPoint`를 다시 호출하지 않는다. 캐시가 없는 진입 경로만 대상 부재를 한 번 직접 조회한다.

카메라는 기존 `ApplyOrientationRotation` 경로를 사용하고 모델 좌표나 SDK ReferenceAxis는 변경하지 않는다. PDF 렌더에서만 1도 초과 부재의 치수·보조선에 로컬 UserAxis를 적용하며, 3D 미리보기의 치수 생성 시점에는 기존 월드축 표시가 유지된다. 정상 부재와 제작도·조립도·설치도에는 UserAxis를 적용하지 않는다.
...
ORIENTATION 우선 판정과 기하 폴백 결과는 진단 로그에 남긴다. 미리보기 카메라는 기존 ORIENTATION 화면 회전을 사용하고, UserAxis 치수는 PDF 행 캡처 후 실측 배율로 그리는 단계에만 적용한다.
...
| 날짜 | 변경 내용 | 작성자 |
|---|---|---|
| 2026-07-23 | 관련: T-079 (기울어진 가공도 부재 축 정렬) — 실기 4건 결과에 따라 ORIENTATION을 최종 판정 우선 신호로 확정하고 LINE Osnap 판정은 값 부재 시 폴백으로 변경. 기존 미리보기 카메라 회전은 유지하며 로컬 UserAxis 치수는 PDF 행 렌더 단계에 한정 | Codex |
| 2026-07-22 | 관련: T-079 (기울어진 가공도 부재 참조축 판정) — LINE Osnap을 5도 방향군으로 묶어 길이 합 주축·월드축 편차·1도 임계값·단일 최장선 비교·ORIENTATION을 로그에 기록. 실제 카메라·치수·출력은 변경하지 않는 1단계 | Codex |
