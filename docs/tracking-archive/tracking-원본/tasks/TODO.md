...
### T-040 — 치수 텍스트 ↔ 치수선 겹침 감지·회피
- **생성일**: 2026-04-22
- **상태**: API 대기
- **관련**: GitHub issue #5, Excel No.53
- **현황**: 앱의 간격 보정만으로 모든 충돌을 피하기 어려워 충돌 결과 조회와 치수 문자 겹침 방지 API 보강을 기다림.
- **할 일**:
  - [ ] API 수정본 수령 후 충돌 판정·회피 적용
  - [ ] 큰 조립체에서 치수 숫자와 치수선 충돌 재검증

### T-084 — UserAxis 2D 치수 문자 각도 보정
- **생성일/착수일**: 2026-07-23
- **상태**: API 대기
- **관련**: GitHub issue #37, Excel No.79
- **현황**: ReferenceAxis 기반 모델·치수선 정렬은 완료. 치수 숫자만 월드 사용자축 각도를 유지하는 문제를 API로 해결해야 함.
- **할 일**:
  - [ ] 화면 수평 고정 또는 문자 회전각 제어 API 수정본 수령
  - [ ] 기울어진 제작도와 정상 제작도의 문자 방향·배치 회귀 검증

### T-048 — 가공도 앵글 모델 촬영 방향 재검토
- **생성일**: 2026-04-28
- **상태**: 분석 필요
- **관련**: GitHub issue #53, Excel No.61·85
- **현황**: EA 상·하 두 뷰와 T자형 회전 오류 보정은 구현돼 있으나 기준면·열린 방향·미러·뷰 교환 규칙을 다시 확정해야 함.
- **할 일**:
  - [ ] 실제 출력에서 두 뷰가 보는 면과 열린 방향 특정
  - [ ] 담당자 기준의 정면·보조면 및 미러 규칙 확정
  - [ ] 확정 규칙으로 기존 구현의 변경 범위 결정

### T-067 — 부재간 접합 각도 자동 측정·표시
- **생성일**: 2026-06-23
- **상태**: API 대기 (자체 구현 시도 병행)
- **관련**: GitHub issue #41, Excel No.52, SDK-008
- **현황**: 공식 API의 8월 배포를 기다리면서 Osnap 기반 접합점·길이축 추정 구현을 예각·사선 부재로 계속 검증·보완.
- **할 일**:
  - [ ] 자체 구현으로 예각·EA·사선 부재 실기 확인
  - [ ] API 수령 후 자체 결과와 비교하고 공식 경로 적용

### T-092 — 모떼기(챔퍼) 형상 검출·표시
- **생성일**: 2026-07-24
- **상태**: API 대기
- **관련**: GitHub issue #42, Excel No.55
- **현황**: 모떼기 판정·필터링 조건을 정리하면서 8월 형상 검출 API 착수·배포를 기다림.

### T-093 — 라운드(필렛) 형상 검출·표시
- **생성일**: 2026-07-24
- **상태**: API 대기
- **관련**: GitHub issue #43, Excel No.54, SDK-006
- **현황**: 라운드 판정·필터링 조건을 정리하면서 8월 형상 검출 API 착수·배포를 기다림.

### T-094 — 2D 실선·은선·모서리·홀 표현 누락
- **생성일**: 2026-07-24
- **상태**: API 대기
- **관련**: GitHub issue #30, Excel No.14, SDK-004
- **현황**: 일부 부재의 2D 실선·은선·모서리·홀 누락 수정본과 배포를 기다림.
- **분리 (2026-07-27)**: 가공도 단면 위주 출력·은선 정책(Excel No.64)은 GitHub issue #117로 분리. 같은 API 수정본을 기다리지만 적용 도면과 검증 대상이 다름.

### T-095 — REV 표 첫 기재행 괘선 보존
- **생성일**: 2026-07-24
- **상태**: API 대기
- **관련**: GitHub issue #33, Excel No.81
- **현황**: 빈 입력 셀 테두리와 고정 템플릿 괘선을 구분하는 API 수정본을 기다림.

### T-096 — 메인 화면 왼쪽 탭·테이블 디자인 개선
- **생성일**: 2026-07-24
- **상태**: 개발 대기
- **관련**: GitHub issue #25, Excel No.73
- **현황**: 탭 선택 상태, 표 헤더·행·테두리, STRU 검색 상자 위치의 디자인 개선 착수 대기.

### T-097 — 긴 Note 텍스트 처리 정책
- **생성일**: 2026-07-24
- **상태**: 분석 필요
- **관련**: GitHub issue #10, Excel No.38
- **현황**: 글자 제한·말줄임·폰트 축소·행 확장 중 생산 기준을 정해야 함.

### T-098 — 작은 치수 텍스트 이동 시 치수선 길이 제어
- **생성일**: 2026-07-24
- **상태**: API 요청 필요 (낮은 우선순위)
- **관련**: GitHub issue #26, Excel No.74
- **현황**: 버그가 아닌 개선 요청이며 아직 API 요청을 전달하지 않음.

### T-099 — 연결부재 이름 풍선 상단·하단 방향 지정
- **생성일**: 2026-07-24
- **상태**: API 요청 필요
- **관련**: GitHub issue #38, Excel No.80
- **현황**: 연결부재 이름 풍선을 상단·하단 영역으로 제한하는 API 요청이 필요함.

### T-100 — 자동 저장 PDF 파일명 정규화
- **생성일**: 2026-07-24
- **상태**: 분석 필요
- **관련**: GitHub issue #49, Excel No.83
- **현황**: 프로젝트·TAG·도면 종류·개정 번호를 포함할지 파일명 규칙을 확정해야 함.
...
| T-012 | 엑셀 템플릿 하이브리드 실험 PoC | REQ-002 / **폐기 (2026-07-28)** — 메인 흐름에 흡수, [DONE](./DONE.md) |
...
- **회사 매핑**: 개발 요청 — 상 3
- **관련**: T-043 (Base Template 정리)와 짝. T-012(엑셀 PoC)는 폐기됐고, 그 검증 결과가 반영된 현행 제작도 엑셀 템플릿 흐름을 비교 기준으로 쓴다
- **회사 원문**:
...
- **세부 (Excel 수신 후)**:
  ...
  - [ ] 현행 템플릿(`제작도_도면_1.xlsx`)과 대조 — 검토자 Excel을 정답지로 활용 가능
