...
docs/코드분석/
  ...
  reach.py           ← 호출 그래프 도달성 — 진입점에서 못 닿는 함수 찾기
  검출 도구.md        ← 위 셋 + grep 으로 죽은 코드를 어떻게 찾았나
...
| `파일별/Form1.DrawingSheets.md` | ✅ 2026-08-22 |
| **파일별 13개 전부** | ✅ **완료 — verify.py 지적 0건** |
| `알고리즘/` | ⬜ |
| `판정/교차검증.md` · `통합 점검.md` · `3차 검증.md` | ✅ 2026-08-23 — 누적 지적 28건 전원 반영 |
| `판정/리팩토링 방안.md` | 🟡 **초안 v1 (2026-08-23)** — D1·D2·D3·D8 결정됨, 나머지는 발표 후 |
| `판정/죽은 코드.md` | ✅ **삭제 완료 2026-08-23 — 실측 1,301줄, 코드 20,578 → 19,143줄** |
| 메서드 요약 주석 | ✅ **2026-08-23 — 118개 추가, 커버리지 318/318 (100%)** |
| `판정/주석 검토.md` (Codex 4차) | ✅ 2026-08-23 — 지적 6건 전원 수용·반영 |
| `알고리즘/` | ⬜ — 발표에 필수는 아님 |
| `장표/` | 🔴 **다음 작업** — 2026-08-27(목) 발표 |
| 실기 확인 (도면 4종 출력) | 🔴 **미완** — 삭제 1,301줄은 빌드로만 검증됨 |
| 옛 문서 75개 정리 | ⬜ **맨 마지막** — 발표 후 |
...
| | |
|---|---|
| ✅ | ~~**죽은 핸들러 8개**~~ → **삭제 완료** (2026-08-23, 커밋 `31e4e37`). 지금 화면의 축·ISO 버튼은 `GlobalViews`로 간다. ⚠ `docs/기능/치수/` 문서 4개는 **여전히 죽은 쪽을 설명 중** — 옛 문서 정리 때 처리 |
| 🔴🔒 | **사내 라이선스 서버 IP·포트가 공개 저장소에 있다** — `60.100.164.177:8901`. `Form1.License.cs:21` · `STATUS.md` · `issues.json` 3곳 = **총 5곳**. 저장소가 PUBLIC이라 커밋 이력에도 남는다. **레포가 사용자 소유가 아니라 private 전환 불가** (2026-08-23 확정) → 레포 소유자와 논의할 사안 |
| ✅ | ~~**`ShowAllDimensions`의 죽은 풍선 배치 블록**~~ → **삭제 완료** (2026-08-23, 커밋 `a4fe0dd`, 실측 272줄). `ShowAllDimensions` 663 → 397줄 |
...
| ✅ | ~~**줌인이 가짜 마우스 휠**~~ → **해소.** 우회 기법은 이미 제거돼 있었고 `SendMessage`·`SetFocus` P/Invoke 선언 + `WM_MOUSEWHEEL`·`WHEEL_DELTA` 상수만 남아 있었다(호출처 0건). 현행 줌은 SDK `View.ZoomRatio`. **2026-08-24 삭제** (죽은 코드 #12, 17줄) |
