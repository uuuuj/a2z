...
5. 🔴 **SDK API를 추측하지 않습니다.** `lib/VIZCore3D.NET.xml`에서 확인하고, 못 찾으면 `(미확인)`을 붙입니다
...
| 이 SDK API 뭐 하는 거지? | `lib/VIZCore3D.NET.xml` |
...
문서를 쓴 뒤 **반드시** 이걸 돌린다. 지적 0건이 되어야 끝난 것이다.

```bash
python docs/코드분석/verify.py Form1.Clash
```

줄번호·SDK API·링크·틀을 기계가 검사한다. **오탐은 거의 없게 조여놨다** — 지적이 나오면 실제 문제일 가능성이 높으니 코드를 확인할 것.

### 교차검증 — Claude Code 담당분을 검토

5개를 다 끝내고 나면, **Claude Code 가 쓴 문서**(`Form1.md`·`Form1.License.md`·`Form1.ExcelTemplate.md`·`Models.md`·`Form1.Drawing2D.md`·`Form1.Dimensions.md`·`Form1.MfgDrawing.md`·`Form1.DrawingSheets.md`)를 검토한다.

**전수 재독은 하지 말 것.** 1~4절은 기계가 잡는다. **5절·6절만** 본다.

| 볼 것 | |
|---|---|
| 5절 알고리즘 | 수식·임계값이 코드와 맞나 (`grep` 으로 숫자 확인) |
| 6절 ② 뗄 수 있다 | **놓친 공유 상태 참조가 없나** → `자동생성/파일 구조.md` 2절과 대조 |
| 6절 ③ 못 뗀다 | 주입·반환값으로 풀리는 걸 구조적 한계로 적지 않았나 |
| 2절 플로우차트 | 빠진 갈래 — 특히 **예외·취소·조기 return** |

🔴 **상대 문서를 직접 고치지 말 것.** `docs/코드분석/판정/교차검증.md` 에 표로 모은다.
