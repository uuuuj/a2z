...
**기본값이 `false`다.** 즉 **지금은 안 쓰인다.** 이유가 코드 주석에 그대로 있다 (L1005~1023).
...
| 무엇을 | 어디로 | 근거 |
|---|---|---|
| 🔑 **PDF 페이지 관리 8함수 + 필드 4개** | `PdfPageAccumulator` | **`Form1` 공유 상태를 하나도 안 쓴다.** 자기 필드 4개와 `vizcore3d.Drawing2D`만 쓴다. 버튼도 없다. 부르는 쪽은 `DrawingSheets`·`MfgDrawing`·`Stru` 셋뿐 |
