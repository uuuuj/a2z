...
- **생성일/착수일**: 2026-07-24
- **상태**: IN_PROGRESS (구현·Debug/Release 빌드 완료, 사내 대용량 모델 실기 검증 대기)
...
### T-087 — 도면 종류별 출력 버튼
- **생성일/착수일**: 2026-07-24
- **상태**: IN_PROGRESS (구현·Debug/Release 빌드 완료, 사내 실기 검증 대기)
...
- **구현**:
  ...
  - [x] Debug·Release 빌드 오류 0개 (기존 경고만 유지)
- **사용자 확인 필요**:
  - [ ] 제작도·조립도·설치도 버튼이 해당 종류의 PDF만 생성하는지
  - [ ] 조립도 여러 장 연속 출력과 현재 작업 후 취소가 정상인지
  - [ ] 마지막 성공 도면은 남고 실패·취소 캔버스는 정리되는지
  - [ ] 기존 가공도·2D 출력·PDF 출력·도면 일괄 출력에 회귀가 없는지
...
- **검증**:
  ...
  - [ ] BOM 수집·간섭검사·일반 PDF·가공도 페이지 각각에서 취소 버튼 실기 확인
  - [ ] 취소 직후 재실행 시 이전 시트·치수·2D 객체가 남지 않는지 확인
  - [ ] 완료된 PDF가 유지되고 요약 PDF 수와 실제 파일 수가 같은지 확인
- **영향 파일**: `A2Z/Form1.cs`, `Form1.BOM.cs`, `Form1.Clash.cs`, `Form1.Stru.cs`, `Form1.MfgDrawing.cs`, 관련 흐름·코드 레퍼런스 문서

### T-084 — 참조축 제작도 UserAxis 치수 문자 화면 정렬
- **생성일/착수일**: 2026-07-23
- **상태**: IN_PROGRESS (1차 우회 구현, 사내 PDF 실기 검증 대기)
- **관련**: 사용자 직접 지시, GitHub issue #37, GitHub issue #19 후속
- **원인 확인**:
  - [x] 참조축 제작도의 모델·치수선은 정상 수평/수직이나 치수 숫자만 약 ±45° 회전하는 실기 화면 확인
  - [x] Release 로그에서 해당 숫자의 UserAxis가 월드 기준 `(0.70711, -0.70711, 0)`·`(0.70711, 0.70711, 0)`임을 확인
  - [x] `Add2DMeasureFrom3DMeasure`가 치수 형상은 참조축 화면에 맞추지만 문자 각도는 월드 사용자축을 유지하는 경계로 원인 범위 축소
  - [x] SDK XML에서 일반 거리 치수 문자 위치 API는 확인했으나 회전각 직접 지정 API는 찾지 못함
- **1차 구현**:
  - [x] `forDrawing2D && drawingReferenceFrame != null`인 제작도 참조축 경로만 `AlignDistanceText=false`
  - [x] 일반 제작도·조립도·설치도·가공도는 기존 `AlignDistanceText=true` 유지
  - [x] 치수 생성 뒤 기본 `AlignDistanceText` 값을 복원하고 적용·복원 진단 로그 추가
- **사용자 확인 필요**:
  - [ ] 기울어진 제작도 평면·정면·측면에서 치수 숫자가 읽기 좋은 화면 방향으로 표시되는지
  - [ ] 숫자 위치·마진과 작은 치수 2단 배치가 깨지지 않는지
  - [ ] 정상 제작도와 다른 종류 도면이 기존 출력과 동일한지
