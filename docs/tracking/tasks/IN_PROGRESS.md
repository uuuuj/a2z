...
- **생성일/착수일**: 2026-07-22
- **상태**: IN_PROGRESS (구현·Debug 빌드 완료, 사내 실기 검증 대기)
- **관련**: 사용자 직접 지시, GitHub issue #23
- **구현**:
  - [x] Sheet 1과 임시 전체 제작도가 같은 BOM 전체 생성 규칙을 쓰도록 공통 헬퍼 추출
  - [x] 기존 선택 시트가 있으면 기존 출력 경로를 그대로 사용
  - [x] 미선택이면 목록의 전체 제작도(`BaseMemberIndex == -1`)를 자동 선택해 출력
  - [x] 목록에 전체 제작도가 없으면 목록을 변경하지 않고 BOM 전체 임시 제작도를 생성해 출력
  - [x] 모델 미로드·BOM 없음·유효 부재 없음 방어 유지
  - [x] Debug 빌드 오류 0건 확인 (기존 경고 7건)
  - [ ] 시트 목록 미선택 상태에서 전체 제작도 출력과 Sheet 1 선택 표시 확인
  - [ ] 시트 목록이 빈 상태에서 전체 제작도 출력 및 목록 미변경 확인
  - [ ] 기존 선택 제작도·조립도·설치도 출력 회귀 확인
- **영향 파일**: `A2Z/Form1.DrawingSheets.cs`, 시트 2D 렌더 흐름 문서, 도면시트 코드 레퍼런스
