...
> 2026-07-26 사내 실기 검증에서 가공도 치수·풍선 종이 절대 정규화(T-089)와 장시간 작업 취소 응답성(T-091)이 정상 확인되어 DONE으로 이동했다.
...
- **생성일**: 2026-07-28
- **착수일**: 2026-07-28
- **상태**: IN_PROGRESS (1단계 구현 완료, 사내 PC 실기 확인 대기)
- **관련**: GitHub issue #64 (REV 표 채우기 로직) · 계획 issue #118 · issue #33 (첫 기재행 괘선 보존, T-095)
- **배경**: 템플릿 슬롯 `Input_170~199`(표제부 REV 이력 표)를 코드가 한 번도 채우지 않아 항상 빈칸이었다. Rev 관리 기능 도입 전에 첫 기재행부터 채워야 한다.
- **단계 분할** (계획 근거: issue #118):
  - [x] **1단계** — 첫 기재행 고정 출력. 공통 헬퍼 `FillRevisionTable`/`BuildCurrentRevisionHistory`(`Form1.ExcelTemplate.cs`)로 `Input_194~199`에 REV.=`0`·DATE=출력일을 넣고 나머지 4칸은 공백 1칸으로 괘선만 보존. 제작도·조립도·설치도는 `GenerateSheetDrawing2D_WithExcelTemplate` 공유 경로 1곳, 가공도는 `BuildMfgPageData`에서 호출 (이번 커밋)
  - [ ] **2단계** — 작성자·검도자·승인자 입력 수단. 도면정보 탭 텍스트박스 3칸 + `Properties.Settings` 영속화 → `data[197~199]` 배선
  - [ ] **3단계** — 리비전 이력 누적. 도면 ID 규칙 확정(STRU명+도면종류+시트번호) → 이력 저장소(출력 폴더 sidecar JSON) → REV 번호 지정 UI → 5행 누적·초과 시 밀어내기
- **결정 대기** (사용자 확인 필요):
  - 작성자·검도자·승인자 입력 수단 — 앱 설정 / 모델 UDA / 출력 다이얼로그. 일괄 출력이 무인 반복이라 **앱 설정 권장**
  - DESCRIPTION 기본 문구 필요 여부 (예: `ISSUED FOR CONSTRUCTION`)
  - REV 번호 증가 조건 — 수동 지정 / 재출력 시 자동 증가. **수동 지정 권장**(재출력마다 REV가 튐)
- **실기 확인 항목**:
  - [ ] 제작도 첫 기재행 6칸 값·괘선 정상 → 정상이면 issue #33(T-095) 종결 검토
  - [ ] DESCRIPTION 등 공백 칸만 괘선이 빠지지 않는지 (`" "` 트릭이 SDK 1.0.26.727에서도 유효한지)
  - [ ] 미사용 이력행(엑셀 40~43행) 괘선이 지워지는 게 의도대로인지 사용자 확인
  - [ ] 가공도 다중 페이지에서 동일 값 출력
- **영향 파일**:
  - `A2Z/Form1.ExcelTemplate.cs` (헬퍼 신설) · `A2Z/Models.cs` (`RevisionEntry`)
  - `A2Z/Form1.DrawingSheets.cs` · `A2Z/Form1.MfgDrawing.cs` (호출 배선)
  - `docs/기능/도면시트/시트 2D 렌더.md` · `docs/기능/가공도/가공도 시트.md`
...
### T-012 — 엑셀 템플릿 하이브리드 실험 (PoC) ⚠️ 폐기 (2026-07-28)

> **폐기 사유**: 실험 목적(엑셀 템플릿을 SDK로 캔버스에 적용할 수 있는가)은 Step 4에서 달성됐고, 그 방식이 메인 제작도 흐름 `GenerateSheetDrawing2D_WithExcelTemplate`에 이미 반영돼 프로덕션에서 돌고 있다. 남은 Step 2·3은 그 전환으로 무의미해졌다.
> PoC 버튼("엑셀 PoC")·핸들러·흐름 문서는 2026-07-28 제거 (#71 작업 중 확인). 제거 시점의 상태 — 짝이던 구 템플릿 `사용자템플릿_엑셀_제작도.xlsx`는 커밋 `0e94aa9`에서 이미 삭제돼 눌러도 에러만 났고, 아래 "영향 파일"의 `groupBox1 너비 +87px`가 실제로는 적용되지 않아 버튼이 그룹박스 밖(X=446 > 폭 443)으로 잘려 화면에도 보이지 않았다.

- **생성일**: 2026-04-20
- **착수일**: 2026-05-12
- **상태**: 폐기 (옛 기록: IN_PROGRESS — Step 1 코드 빌드 통과, 사용자 사내 PC 실기 검증 대기)
- **관련**: REQ-002
...
- **관련 docs**: 흐름 문서 `docs/기능/도면시트/엑셀 템플릿 PoC.md`는 버튼과 함께 삭제 (내용은 git 이력에 보존). 현행 엑셀 템플릿 흐름은 [시트 2D 렌더.md](../../기능/도면시트/시트%202D%20렌더.md)
