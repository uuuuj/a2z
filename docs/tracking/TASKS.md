...
사용자가 회사 doc과 별개로 정리한 "수정완료 확인 대기" 항목 — 검토자·회사 답변 필요.

| # | 사용자 표현 요지 | 매핑 ID | 우리 시각 | 차이 / 주의 |
|---|---|---|---|---|
| 1 | 풍선·심볼 정의 | T-054 | TODO | 사용자가 정의 명시. 검토자 확정 받으면 docs 등록 |
| 2 | Item 기준부재 Z-MAX 정렬 | T-056 (DONE 검증보고서) | 부분 일치 | **차이**: 코드 `BBox.MaxZ` vs 명세 `Osnap.Z`. 일반 형상에선 동등, 경사·곡면에서 차이. **결정 (2026-05-05)**: BBox 유지, § 7 단답으로 회사 회신 |
| 3 | 치수 추출 3회 이상 정상 | T-016 (BLOCKED 간헐) | 사용자 재현 정상 | T-016 CLOSE 검토 필요 vs BLOCKED 유지 |
| 4 | 단일 부재 + 연결성 검사 | T-023 v3 (DONE) | 일치 | — |
| 5 | 가공도 보조선 가는 실선 | T-046 (DONE) | 일치 | — |
| 6 | 보조선 Osnap에서 일정 거리 띄움 | T-046 확장 (DONE) | 사용자 우려 합리적 | **주의**: 치수선이 모델 안쪽에 배치되는 케이스에서 gap이 모델 안으로 진입 가능. 재현 1~2건 받아 분기 추가 검토 필요 |
| 7 | 기준부재 표현방식 | T-042 (DONE) | 사양 재정리 중 | 사용자 새 아이디어: Sheet 1 기준부재 = **LCA 노드 이름** (모든 기준부재를 포함하며 모델트리상 가장 가까운 공통 조상). 사양 확정 후 신규 작업으로 진행 |
| 8 | Sheet 1 포함부재 1, 2, 3... | T-052 (DONE) | 일치 | — |
| 9 | 중복 Sheet 삭제 후 번호 재채번 | T-053 v2 (DONE) | 일치 | — |
| 10 | 치수 추출 후 3D 뷰 깨끗 정리 | T-029 + T-049 (DONE) | 일치 | — |
| 11 | 3D View 축 표시 | T-050 (DONE) | 일치 | — |

→ T-054, T-016은 잔여 카테고리에서 본 검토 대기로 이동.
...
| **T-060** | **보조선 시작점이 모델과 겹쳐 보이는 케이스 회피** | 사용자 본인 발견 / TODO (재현 케이스 대기, 신규) |
