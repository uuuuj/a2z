...
### 검토 대기 (사용자 정리 8건, 2026-05-04 추가 → 2026-05-05 본인 개선 분리)
사용자가 회사 doc과 별개로 정리한 "수정완료 확인 대기" 항목 — 검토자·회사 답변 필요. **회사 doc 매핑 있는 항목만 유지**.

| # | 사용자 표현 요지 | 매핑 ID | 우리 시각 | 차이 / 주의 |
|---|---|---|---|---|
| 1 | 풍선·심볼 정의 | T-054 (TODO, 회사 수정 후 확인 1) | TODO | 사용자가 정의 명시. 검토자 확정 받으면 docs 등록 |
| 2 | Item 기준부재 Z-MAX 정렬 | T-056 (DONE 검증보고서, 회사 완료 5 + 수정 후 확인 2) | 부분 일치 | **차이**: 코드 `BBox.MaxZ` vs 명세 `Osnap.Z`. 일반 형상에선 동등, 경사·곡면에서 차이. **결정 (2026-05-05)**: BBox 유지, § 7 단답으로 회사 회신 |
| 3 | 가공도 보조선 가는 실선 | T-046 (DONE, 회사 긴급상 10) | 일치 | — |
| 4 | 보조선 Osnap에서 일정 거리 띄움 | T-046 확장 (DONE, T-046 본체 묶음) | 사용자 우려 합리적 | **주의**: 치수선이 모델 안쪽에 배치되는 케이스에서 gap이 모델 안으로 진입 가능. 재현 1~2건 받아 분기 추가 검토 필요 |
| 5 | 기준부재 표현방식 | T-042 (DONE, 회사 긴급최우선 1) | 사양 재정리 중 | 사용자 새 아이디어: Sheet 1 기준부재 = **LCA 노드 이름** (모든 기준부재를 포함하며 모델트리상 가장 가까운 공통 조상). 사양 확정 후 신규 작업으로 진행 |
| 6 | Sheet 1 포함부재 1, 2, 3... | T-052 (DONE, 회사 긴급하 3) | 일치 | — |
| 7 | 중복 Sheet 삭제 후 번호 재채번 | T-053 v2 (DONE, 회사 긴급하 4) | 일치 | — |
| 8 | 3D View 축 표시 | T-050 (DONE, 회사 긴급하 1) | 일치 | — |

**2026-05-05 본인 개선으로 이동** (회사 doc 매핑 없음):
- T-016 (치수 추출 3회 이상 간헐 버그) — 사용자 피드백 출처
- T-023 v3 (단일 부재 + 연결성 1덩어리 판정) — 사용자 직접 지시 출처
- T-029 (치수추출 후 3D 뷰 깨끗) — 사용자 직접 지시 출처. T-049(회사 긴급중 3, 치수 캐시 docs)와 묶음이었으나 출처 분리

### 본인 개선 사항 (사용자 본인 발견 — 회사 doc 외)
사용자가 직접 발견·요청한 개선 항목 (FB·REQ 출처 포함, 사용자가 일관되게 본인 영역으로 분류).
...
| T-016 | 치수 추출 3회 이상 시 반복 누적 버그 (간헐) | 사용자 피드백 / BLOCKED (재현 대기, 2026-05-05 검토 대기에서 이동) |
| T-023 | 치수추출 사전조건 (연결성 1덩어리 판정) | 사용자 직접 / DONE (2026-05-05 검토 대기에서 이동) |
| T-028 | 치수 로직 4경로 통합 | 사용자 직접 / IN_PROGRESS (실기 확인 대기) |
| T-029 | 치수추출 버튼은 3D 뷰 치수 렌더링 안 함 | 사용자 직접 / DONE (2026-05-05 검토 대기에서 이동, T-049와 묶음 분리) |
