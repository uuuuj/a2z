...
### Softhills API 확인요청 (6건, 외부 추적 — 우리 작업 아님)
| # | 항목 | 추적 ID |
|---|---|---|
...
- **영향 파일**: A2Z/Form1.DrawingSheets.cs, docs/기능/도면시트/시트 2D 렌더.md
...
### T-067 — 제작도 비-90° 각도 표시
- **완료일**: 2026-06-23 (커밋 `f70cefa`)
- **관련**: 사용자 직접 지시 (3개 요청 중 2번째 — 90°가 아닌 곳 각도 표현)
- **요약**: `MarkNonRightAngles` 신규. 제작도 X/Y/Z 뷰에서 LINE osnap 세그먼트를 3D 꼭짓점으로 묶어 화면 투영 사잇각을 구해 90의 배수가 아니면 `AddCustom3PointAngle`로 표시. `ShowAllDimensions` 직후 호출로 같은 Measure→2D 파이프라인 탑승. 적대적 검증으로 3D 꼭짓점 그룹핑·ID 단위 스타일 반영. **실측 확정 필요**: AddCustom3PointAngle 인자 순서(45° 테스트), 내부선 포함 여부.
