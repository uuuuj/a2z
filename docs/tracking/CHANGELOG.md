...
## 2026-05-05 — 검토 대기 카테고리 재분류 (회사 doc 매핑 없는 3건 본인 개선으로 이동)
...
**배경**: 사용자가 검토 대기 11건 중 회사 doc 매핑 없는 항목을 본인 개선으로 일원화 요청. "사용자 실기 확인중에서 회사 문서에 없는것들은 전부 내 개인 개선사항으로 옮겨".

**이동 항목** (3건):
- **T-016 — 치수 추출 3회 이상 시 반복 누적 버그 (간헐)**: 사용자 피드백 출처, BLOCKED 재현 대기
- **T-023 v3 — 치수추출 사전조건 (연결성 1덩어리 판정)**: 사용자 직접 지시, DONE
- **T-029 — 치수추출 버튼은 3D 뷰 치수 렌더링 안 함**: 사용자 직접 지시, DONE. T-049(회사 긴급중 3, 치수 캐시 docs)와 묶음이었으나 출처가 달라 분리

**유지 항목** (검토 대기 8건):
- T-054 (회사 수정 후 확인 1), T-056 (완료 5+수정 후 확인 2), T-046 (긴급상 10, 확장 본체 묶음), T-042 (긴급최우선 1), T-052 (긴급하 3), T-053 (긴급하 4), T-050 (긴급하 1)

**Tracking 갱신**:
- TASKS.md 머릿주석 검토 대기 표 8건으로 축소 + 매핑 ID에 회사 doc 매핑 정보 명시 + 이동 메모
- TASKS.md 본인 개선 사항 표에 T-016/T-023/T-029 추가 (11건 → 14건)
