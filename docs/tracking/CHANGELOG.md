...
- 전체 `{View_1}` 기준은 SDK가 풍선을 셀 밖으로 밀어 템플릿을 벗어나므로, 실제 모델 중심과 fit 70% 캔버스 영역에서 `offset=10px`로 최종 정렬
- 제작도·설치도 연결 이름 노트를 부재번호 풍선보다 먼저 생성해 동일한 모델 fit 70% 영역으로 정렬. 이름 지시선의 실제 접합점 Target은 유지
