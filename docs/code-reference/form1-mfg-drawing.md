...
**경로**: `A2Z/Form1.MfgDrawing.cs` (약 3,825 라인)
...
| `SplitMfgIntoPages` | L89 | 활성, 페이지당 5부재 분할 |
| `SnapshotBomRows` | L114 | 활성, `lvDrawingBOMInfo` 행을 메모리로 1회 복사. 요약행(Row 0)을 첫 원소로 포함 (#67) |
| `BuildMfgPageData` | L142 | 활성, 페이지 엑셀 슬롯 데이터 구성 (PAINT CODE Input_166, REV 표 첫 기재행 Input_194~199 — 공통 `FillRevisionTable`, 부재명 Input_241~245, BOM 8×25 Input_4~240 — 1행=요약행·2~25행=데이터행 24개, SDK 1.0.26.723의 Input 200+ 수정 반영) |
| `CheckMfgCancellation` | L69 | 수동·일괄 가공도 진행 문구 갱신, UI 메시지 처리, 안전 체크포인트 취소 예외 발생 |
| `GetSprefValue` | L2920 | 활성 |
| `ParseOrientation` | L3138 | 활성 |
| `TryResolveMfgHoleThroughAxis` | L1619 | `ThicknessCenter` 차이 → 원형 홀 `CircleCenter` → 검증된 AxisZ 순으로 관통축 결정 |
| `BuildMfgPendingNotes` | L1819 | Hole/SlotHole을 EA 뷰별 배정 후 규격 그룹화, EarthBoss는 첫 뷰 1회 |
| `PromoteMfgSmallPendingDimensions` | L749 | 캡처 전 짧은 치수 승격을 확정해 fit 예약과 실제 치수가 같은 레벨을 사용 |
| `GetMfgSameSideDimensionEnvelopeCanvas` | L780 | 풍선과 같은 화면 위·아래 쪽에 있는 치수 오프셋만 종이 mm로 계산 |
| `GetMfgAnnotationBudgetCanvas` | L849 | EA는 공통 예약 높이를 우선 사용하고 단일 뷰는 현재 치수·풍선으로 모델 fit 전 주석 영역 예약 |
| `GetMfgEaSharedAnnotationBudgetCanvas` | L867 | 두 EA 풍선 목록 중 최대 행 수와 최대 3단 치수 외곽으로 공통 주석 예약 높이 계산 |
| `CalculateMfgAnnotationBudgetCanvas` | L891 | 치수선·문자·풍선 간격·행 높이의 종이 절대 예약 높이 합산 |
| `GetMfgOrientationAxisVector` | L3533 | 활성 ReferenceAxis와 같은 로컬 X/Y/Z 축 벡터를 UserAxis 치수용 `Vertex3D`로 반환 |
| `DetectMfgAxis` | L3633 | ORIENTATION 부재 시 사용할 LINE Osnap 5도 방향군·길이 합 폴백 판정 |
| `LogMfgAxisDetection` | L3727 | ORIENTATION 우선 최종 판정과 기하 대조 결과 진단 로그 |
