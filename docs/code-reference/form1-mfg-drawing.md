...
**경로**: `A2Z/Form1.MfgDrawing.cs` (약 3,605 라인)
...
| 핸들러 | 라인 | 흐름 문서 |
|---|---:|---|
| <a id="btnMfgDrawingSheet_Click"></a>`btnMfgDrawingSheet_Click` | L2645 | [가공도 시트 PDF 출력](../기능/가공도/가공도%20시트.md) |
...
| `GetSprefValue` | L2701 | 활성 |
| `ParseOrientation` | L2919 | 활성 |
| `TryResolveMfgHoleThroughAxis` | L1507 | `ThicknessCenter` 차이 → 원형 홀 `CircleCenter` → 검증된 AxisZ 순으로 관통축 결정 |
| `BuildMfgPendingNotes` | L1707 | Hole/SlotHole을 EA 뷰별 배정 후 규격 그룹화, EarthBoss는 첫 뷰 1회 |
| `PromoteMfgSmallPendingDimensions` | L718 | 캡처 전 짧은 치수 승격을 확정해 fit 예약과 실제 치수가 같은 레벨을 사용 |
| `GetMfgSameSideDimensionEnvelopeCanvas` | L749 | 풍선과 같은 화면 위·아래 쪽에 있는 치수 오프셋만 종이 mm로 계산 |
| `GetMfgAnnotationBudgetCanvas` | L818 | 치수선·문자·풍선 간격·행 높이를 합산해 모델 fit 전 주석 영역 예약 |
| `GetMfgOrientationAxisVector` | L3314 | 활성 ReferenceAxis와 같은 로컬 X/Y/Z 축 벡터를 UserAxis 치수용 `Vertex3D`로 반환 |
| `DetectMfgAxis` | L3414 | ORIENTATION 부재 시 사용할 LINE Osnap 5도 방향군·길이 합 폴백 판정 |
| `LogMfgAxisDetection` | L3508 | ORIENTATION 우선 최종 판정과 기하 대조 결과 진단 로그 |
