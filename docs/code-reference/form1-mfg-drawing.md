...
**경로**: `A2Z/Form1.MfgDrawing.cs` (약 3,645 라인)
...
| 핸들러 | 라인 | 흐름 문서 |
|---|---:|---|
| <a id="btnMfgDrawingSheet_Click"></a>`btnMfgDrawingSheet_Click` | L2685 | [가공도 시트 PDF 출력](../기능/가공도/가공도%20시트.md) |
...
| `GetSprefValue` | L2741 | 활성 |
| `ParseOrientation` | L2959 | 활성 |
| `TryResolveMfgHoleThroughAxis` | L1547 | `ThicknessCenter` 차이 → 원형 홀 `CircleCenter` → 검증된 AxisZ 순으로 관통축 결정 |
| `BuildMfgPendingNotes` | L1747 | Hole/SlotHole을 EA 뷰별 배정 후 규격 그룹화, EarthBoss는 첫 뷰 1회 |
| `PromoteMfgSmallPendingDimensions` | L719 | 캡처 전 짧은 치수 승격을 확정해 fit 예약과 실제 치수가 같은 레벨을 사용 |
| `GetMfgSameSideDimensionEnvelopeCanvas` | L750 | 풍선과 같은 화면 위·아래 쪽에 있는 치수 오프셋만 종이 mm로 계산 |
| `GetMfgAnnotationBudgetCanvas` | L819 | EA는 공통 예약 높이를 우선 사용하고 단일 뷰는 현재 치수·풍선으로 모델 fit 전 주석 영역 예약 |
| `GetMfgEaSharedAnnotationBudgetCanvas` | L837 | 두 EA 풍선 목록 중 최대 행 수와 최대 3단 치수 외곽으로 공통 주석 예약 높이 계산 |
| `CalculateMfgAnnotationBudgetCanvas` | L861 | 치수선·문자·풍선 간격·행 높이의 종이 절대 예약 높이 합산 |
| `GetMfgOrientationAxisVector` | L3354 | 활성 ReferenceAxis와 같은 로컬 X/Y/Z 축 벡터를 UserAxis 치수용 `Vertex3D`로 반환 |
| `DetectMfgAxis` | L3454 | ORIENTATION 부재 시 사용할 LINE Osnap 5도 방향군·길이 합 폴백 판정 |
| `LogMfgAxisDetection` | L3548 | ORIENTATION 우선 최종 판정과 기하 대조 결과 진단 로그 |
