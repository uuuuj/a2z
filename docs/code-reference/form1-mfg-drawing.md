...
**경로**: `A2Z/Form1.MfgDrawing.cs` (약 3,435 라인)
...
| 핸들러 | 라인 | 흐름 문서 |
|---|---:|---|
| <a id="btnMfgDrawingSheet_Click"></a>`btnMfgDrawingSheet_Click` | L2475 | [가공도 시트 PDF 출력](../기능/가공도/가공도%20시트.md) |
...
- `GetNodeHoleInfo`의 두께 중심점 차이를 우선 관통축으로 사용하고, EA 두 뷰의 실제 깊이축과 `abs(dot)`을 비교한다. 방향 배정 뒤 뷰별로 Hole/SlotHole 규격과 개수를 그룹화해 `PendingNotes`·`SecondaryPendingNotes`에 저장한다.
...
- `DrawMfgDimsAtScale`가 남긴 실제 최대 치수 오프셋 바깥에 첫 풍선을 종이 6mm 간격으로 두고, 여러 풍선은 종이 8mm 행 간격으로 각 EA 슬롯의 바깥쪽에 쌓는다.
...
- 실제 사용한 최대 치수 오프셋을 `pose.DimensionEnvelopeOffset`에 남겨 풍선 배치가 같은 종이 절대 경계를 사용하게 한다.
...
| `GetSprefValue` | L2531 | 활성 |
| `ParseOrientation` | L2749 | 활성 |
| `TryResolveMfgHoleThroughAxis` | L1372 | `ThicknessCenter` 차이를 우선 사용하고 AxisZ·AxisX×AxisY 교차검증 |
| `BuildMfgPendingNotes` | L1537 | Hole/SlotHole을 EA 뷰별 배정 후 규격 그룹화, EarthBoss는 첫 뷰 1회 |
| `GetMfgOrientationAxisVector` | L3144 | 활성 ReferenceAxis와 같은 로컬 X/Y/Z 축 벡터를 UserAxis 치수용 `Vertex3D`로 반환 |
| `DetectMfgAxis` | L3244 | ORIENTATION 부재 시 사용할 LINE Osnap 5도 방향군·길이 합 폴백 판정 |
| `LogMfgAxisDetection` | L3338 | ORIENTATION 우선 최종 판정과 기하 대조 결과 진단 로그 |
