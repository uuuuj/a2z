...
- 페이지마다 북쪽 화살표 배치(2026-07-20): `View_6`=N 화살표·`View_7`=ISO 화살표를 `PlaceImageInTemplateArea`(제작도 공용, RenderTemplate 캘리브레이션 포함)로 배치, 슬롯 없으면 생략. `EnsureViewAreasCache`는 중복 View 태그 방어(첫 위치 사용+경고). ⚠ **View_8 이상 금지** — import 시 SDK 내부 메모리 손상 → 직후 캡처 AccessViolation (2026-07-20 실측, View 번호는 1~7만).
