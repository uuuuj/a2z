...
| `[API 개발]` | `상태: API 개발` | 새 API 추가 또는 기존 SDK 수정이 필요하다고 확인됨 |
...
- API 관련 기본 흐름: `API 확인 → API 개발 → API 대기 → 개발 대기 또는 실기 확인`.
- API 확인 결과 기존 API로 구현 가능하면 `API 개발`을 거치지 않고 `개발 대기`로 변경한다.
