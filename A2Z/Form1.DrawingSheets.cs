    ...
    public partial class Form1
    {
        ...
        private void btnExportAllPDF_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                for (int i = 0; i < lvDrawingSheet.Items.Count; i++)
                {
                    ...
                    try
                    {
                        for (int canvasId = 1; canvasId <= 10; canvasId++)
                            vizcore3d.Drawing2D.View.RemoveCanvasBy2DView(canvasId);
                    }
                    catch { }
        ...
        private void GenerateSheetDrawing2D(DrawingSheetData sheet)
        {
            try
            {
                ...
                vizcore3d.Drawing2D.Template.RenderTemplateOnGridStructure(table2, 2, 3);
        ...
        private int RenderSheetViewForDrawing(int row, int col, string viewDirection, DrawingSheetData sheet, float targetHeight = 0f)
        {
            ...
            if (isIsoFullView)
            {
                // ── Pass 1: 전체 BOM 부재 보이기 → 가는 점선으로 캡처 (배경) ──
                vizcore3d.BeginUpdate();
                vizcore3d.Object3D.Show(VIZCore3D.NET.Data.Object3DKind.ALL, false);
                vizcore3d.Object3D.Show(allBomIndices, true);
                vizcore3d.EndUpdate();

                vizcore3d.Drawing2D.Object2D.Set2DViewCreateObjectItemLineWidth(0.15f);
                ...
                // ── Pass 2: 전체 BOM 부재 보이기 (동일 카메라) → 시트 부재만 굵은 실선으로 캡처 ──
                // 시트 부재만 표시하되 카메라/줌은 변경하지 않음
                vizcore3d.BeginUpdate();
                vizcore3d.Object3D.Show(VIZCore3D.NET.Data.Object3DKind.ALL, false);
                vizcore3d.Object3D.Show(sheet.MemberIndices, true);
                vizcore3d.EndUpdate();
                ...
                // 배경 객체를 먼저 셀에 배치 (전체 BOM 바운딩 박스 기준)
            ...
            // A4 6등분 셀 중앙에 배치 + 셀 크기에 맞춰 자동 스케일
            vizcore3d.Drawing2D.Object2D.FitObjectToGridCellAspect(row, col, objId,
                VIZCore3D.NET.Data.GridHorizontalAlignment.Center,
                VIZCore3D.NET.Data.GridVerticalAlignment.Middle);

            // 6. 스케일 조정 (Note/Measure 변환 전에 수행 — 풍선 위치가 조정된 스케일에 맞도록)
            if (targetHeight > 0)
            {
            ...
            // Sheet 2+ ISO: 배경 객체도 주 객체와 동일한 최종 스케일 적용
            if (bgObjId >= 0)
            {
                float finalScale = vizcore3d.Drawing2D.Object2D.GetObjectScale(objId);
                vizcore3d.Drawing2D.Object2D.RescaleObject(bgObjId, finalScale);
            }
