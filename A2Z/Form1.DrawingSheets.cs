...
namespace A2Z
{
    public partial class Form1
    {
        ...
        private void GenerateSheetDrawing2DCore(DrawingSheetData sheet)
        {
            ...
            // 도면 생성 — 엑셀 템플릿 경로 (P2 이후 유일한 경로).
            // (2026-08-23 죽은 코드 제거) 옛 무템플릿 직접 그리기 본문 약 330줄 +
            //   RenderSheetViewForDrawing(344줄) + EstimateFitScaleForCell(54줄) +
            //   분기 스위치 UseExcelTemplate(true 하드코딩, false로 바꾸는 코드 없음 → 도달 불가) 삭제.
            //   근거: docs/코드분석/판정/죽은 코드.md #7
        ...
        private void GenerateSheetDrawing2D_WithExcelTemplate(DrawingSheetData sheet)
        {
            ...
            try
            {
                ...
                Dictionary<int, VIZCore3D.NET.Data.CameraDirection> cameraMap = new Dictionary<int, VIZCore3D.NET.Data.CameraDirection>
                {
                    ...
                    { 2, VIZCore3D.NET.Data.CameraDirection.Z_PLUS   },   // 카메라 +Z, Looking -Z
                    { 3, VIZCore3D.NET.Data.CameraDirection.X_PLUS   },   // 카메라 +X, Looking -X
                    { 4, VIZCore3D.NET.Data.CameraDirection.Y_MINUS  },   // 카메라 -Y, Looking +Y
                };
                ...
                for (int i = 0; i < viewAreas.Count; i++)
                {
                    ...
                    if (viewDir != "ISO" && sheet.BaseMemberIndex == -2)
                    {
                        ...
                        xraySelectedNodeIndices = new List<int>(sheet.MemberIndices);
                        // keepCamera: 표준 카메라 뒤 적용한 ORIENTATION/참조축 회전을 유지한다.
                        //   ShowAllDimensions가 다시 MoveCamera하면 모델과 보조선·치수의 2D 변환이 어긋난다.
        ...
        /// 옛 EstimateFitScaleForCell(GridStructure 셀 기반, 2026-08-23 삭제)과 동일 알고리즘 — 입력을 viewArea 영역으로.
