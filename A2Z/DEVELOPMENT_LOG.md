...
### Phase 12: 도면시트 선택 시 BOM정보 자동 필터링 + 풍선 오프셋 조정

**[요청 1]** "도면정보에서 데이터 누르면 그 데이터에 관련된 부재만 BOM정보에 뜨게"

**[구현 — 2곳 수정]**

1. **btnCollectBOMInfo_Click (Line 1489~1528)** — 시트 필터링 로직 추가
   - `lvDrawingSheet.SelectedItems`에서 선택된 시트의 `DrawingSheetData` 확인
   - `MemberIndices` (Body 인덱스)를 HashSet으로 변환
   - Body 노드 전체를 가져와 이진탐색으로 Body→부모 Part 매핑
   - 시트 Body의 부모 Part 인덱스만 `allowedPartIndices`에 추가
   - `partNodes`를 해당 Set으로 필터링 → 이후 UDA 조회/그룹핑/bomInfoNodeGroupMap은 변경 없음
   - 시트 미선택 시 전체 Part 대상 (하위호환)

2. **LvDrawingSheet_SelectedIndexChanged (Line 6673)** — 자동 BOM 수집
   - 핸들러 끝에 `btnCollectBOMInfo_Click(sender, e)` 호출 추가
   - 도면시트 선택 → X-Ray + 치수 추출 + BOM 자동 수집까지 원스텝

**[요청 2]** "홀이랑 슬롯홀 표현이 부재랑 너무 붙어있어서 멀어져도 될 거 같아"

**[구현 — 2곳 수정]**

1. **ShowBalloonNumbers (Line 4499)** — ISO 풍선 오프셋
   - `balloonOffset`: 25f → **50f**

2. **ShowAllDimensions (Line 6242)** — 홀/슬롯홀 풍선 오프셋
   - `baseOffset`: `modelDiag * 0.25f, 50f` → **`modelDiag * 0.35f, 70f`**
