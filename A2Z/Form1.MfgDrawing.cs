    ...
    public partial class Form1
    {
        ...
        private int RenderMfgViewForDrawing(int row, int col, int bomIndex)
        {
            ...
            if (measureIds.Count > 0)
            {
                ...
                // T-040 (2026-05-13 v2): ≤13mm 치수 텍스트 시프트 (ISO 제외)
                //  - 뷰 max ≤ 100mm 이면 작은 모델로 보고 전체 skip
                //  - 치수축이 화면 가로(H)면 위(up) / 세로(V)면 오른쪽(right) 으로 캔버스 3mm 시프트
                ...
                if (viewDirection != "ISO")
                {
                    ...
                    if (canvasScaleMfg > 0.0001f)
                    {
                        ...
                        if (maxEstDist <= 100f)
                        {
                            DiagLog($"T-040 Mfg TextShift view={viewDirection} skip (maxEstDist={maxEstDist:F1}mm <= 100mm)");
                        }
                        else
                        {
                            float modelShift = 3f / canvasScaleMfg;
                            int shiftedCount = 0;
                            foreach (var measure in measures)
                            {
                                if (!measure.Visible) continue;
                            ...
                            DiagLog($"T-040 Mfg TextShift view={viewDirection} canvasScale={canvasScaleMfg:F4} modelShift={modelShift:F1}mm maxEstDist={maxEstDist:F1}mm shifted={shiftedCount}");
                        }
                    }
                }
